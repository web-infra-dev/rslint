...
async function test() {
  ...
  fs.writeFileSync(tsconfigPath, JSON.stringify({
    compilerOptions: {
      ...
      emitDecoratorMetadata: true // Both are true now
    },
    include: ["test.ts"]
  }));
...
class A {
  ...
  console.log('emitDecoratorMetadata: true, experimentalDecorators: true');
  ...
  try {
    ...
    console.log('Expected: Should NOT report error since both flags are true');
  } catch (err) {
