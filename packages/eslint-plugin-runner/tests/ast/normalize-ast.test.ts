...
import type { ESTreeNode } from '../../src/linter/context.js';

function parseAndNormalize(filePath: string, text: string): ESTreeNode {
  const parsed = parseSync(filePath, text, { sourceType: 'module' });
  const ast = (
...
describe('R2: oxc UTF-16 offset → Go-friendly UTF-8 byte offset', () => {
  test('diagnostic startPos is in UTF-8 bytes after lintFile drains', async () => {
    ...
    const text = '// ➜\n// ➜\n    let z = null;\n';
    const { lintFile } =
      await import('../../src/linter/ecma-language-plugin.js');
...
describe('buildLineStartOffsets — ECMAScript line terminators', () => {
  ...
  test('source-code re-export points at the same implementation', async () => {
    const { buildLineStartOffsetsLocal } =
      await import('../../src/source-code/source-code.js');
    for (const sample of [
      'a',
      'a\nb',
      'a\r\nb',
...
// ─────────────────────────────────────────────────────────────────────
// oxc-parser `range: true` integration — verify normalize doesn't
// reallocate the range array oxc already supplied.
// ─────────────────────────────────────────────────────────────────────

describe('normalize-ast — preserves oxc-emitted range', () => {
  test('range:true gives every node a [start, end] pair before normalize runs', () => {
    const text = 'const x = 1;';
    const parsed = parseSync('a.ts', text, {
      sourceType: 'module',
      range: true,
    });
    ...
    expect(ast.range).toEqual([0, 12]);
    const decl = ast.body[0] as Record<string, unknown>;
    expect(decl.range).toEqual([0, 12]);
  });

  test('normalize preserves the SAME range array reference (no realloc)', () => {
    const text = 'const x = 1;';
    const parsed = parseSync('a.ts', text, {
      sourceType: 'module',
      range: true,
    ...
    const programRangeBefore = (ast as unknown as { range: number[] }).range;
    const declRangeBefore = (
      ast as unknown as { body: Array<{ range: number[] }> }
    ).body[0].range;
    const lso = buildLineStartOffsets(text);
    normalizeAst(ast, lso, text);
    // Same array identity → the `node.range = [node.start, node.end]`
    // branch was skipped, no JS-side allocation.
    expect((ast as unknown as { range: number[] }).range).toBe(
      programRangeBefore,
    );
    expect(
      (ast as unknown as { body: Array<{ range: number[] }> }).body[0].range,
    ).toBe(declRangeBefore);
  });

  test('normalize still adds loc (oxc does not emit loc natively)', () => {
    const text = 'const x = 1;\nconst y = 2;';
    const parsed = parseSync('a.ts', text, {
      sourceType: 'module',
      range: true,
