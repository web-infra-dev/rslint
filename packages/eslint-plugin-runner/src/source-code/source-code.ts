...
/**
 * ESLint-compatible `SourceCode` for ESLint plugins running inside the
 * runner Worker. Empirically pinned against ESLint v10 — see
 * `tests/conformance/audit-api.test.ts` for the surface audit.
 *
 * Why this is its own module: `createSourceCode` is ~700 LoC of
 * token/comment/scope/spacing/visitor-keys logic that's largely
 * orthogonal to `RuleContext`. Splitting keeps `context.ts` focused on
 * `report()` wiring + diagnostic capture; this file owns AST querying.
 *
 ...
 * SourceCode instance):
 *
 *   - tokens / comments   tokenize() runs on first token-API call
 *   - scopeManager        scope-factory thunk fires on first scope call
 */

// Use the runner's canonical visitor-key table (built from
// `@typescript-eslint/visitor-keys`, see `visitor-keys.ts`). The
// bare `eslint-visitor-keys` `KEYS` covers only ESTree, missing
// every TS-specific node type (TSEnumDeclaration, TSInterfaceDeclaration,
// TSAsExpression, TSMappedType, …). Using it for `sourceCode.visitorKeys`
// silently dropped TS subtrees from `for (const k of sc.visitorKeys[node.type])`
// walks, and made `getNodeByRangeIndex` stop descending into TS bodies.
import {
...
export interface SourceCodeBuildInput {
   ...
   * passing it in — both oxc-parser and downstream offset-aware code
  ...
  hasBOM?: boolean;
  /**
   * Enable JSX lexing. Two trigger conditions in upstream callers:
   *   1. `parserOptions.ecmaFeatures.jsx === true` (explicit flag)
   *   2. file extension `.jsx` / `.tsx` (oxc auto-infers JSX for
   *      these regardless of the flag, so the tokenizer must follow)
   *
   * Pure JS / TS files set this to false — the JSX state machine is
   * fully gated by this flag so non-JSX files pay zero overhead. See
   * `tokenizer.ts::TokenizeOptions.jsx` for the lexer-side contract.
   */
  jsx?: boolean;
  /**
   * Optional oxc-parser-emitted comments. When provided, `getAllComments`
   ...
   *
   * Shape from oxc-parser: `{ type: 'Line' | 'Block', value, start, end }`.
...
/**
 * Build ESLint-shape `Comment[]` from oxc-parser's compact comment list.
 *
 ...
 * O(text_length) full text scan the standalone tokenizer would have
...
function buildCommentsFromParsed(
  ...
  const out: Comment[] = [];
  // oxc-parser emits the shebang line as `type: 'Line'` at offset 0.
  // ESLint's `getAllComments` contract reports it as `type: 'Shebang'`
  // with the same `value`/`range`/`loc`. Detect & rewrite the first
  // comment when (1) the file starts with `#!` and (2) the first
  // emitted comment begins at offset 0 — we don't want to confuse a
  // real `//` line on line 1 with a shebang.
  const hasShebang =
  ...
  // Synthesize a `Shebang` entry when the file starts with `#!` but no
  // offset-0 comment was emitted. oxc surfaces the shebang as a `Line`
  // comment ONLY for `.js`-family files (relabeled above); for the TS
  // family (`.ts`/`.mts`/`.cts`/`.tsx`) it puts the shebang solely on
  // `program.hashbang` and leaves `comments` empty, so the relabel loop
  // produces nothing. ESLint v10's `getAllComments()` always reports a
  // leading `#!` as a `Shebang` token regardless of language, so we
  // build it from the source text to match (value strips `#!`, range
  // spans to the first line terminator — same shape espree produces).
...
export function createSourceCode(input: SourceCodeBuildInput): SourceCode {
  ...
  const ensureTokens = (): { tokens: Token[]; comments: Comment[] } => {
    if (_tokens == null) {
      const r = tokenize(text, getLso(), { jsx: input.jsx === true });
      _tokens = r.tokens;
      // Only adopt the tokenizer's comments if no cheap oxc-derived
      // comment list was already built — we don't want to discard the
      // pre-populated _comments array, which would re-pay the loc
      // conversion cost. Both paths produce equivalent ESLint-shape
      // Comment objects; the tokenizer's just goes via a full text scan.
      if (_comments == null) _comments = r.comments;
    }
    return { tokens: _tokens, comments: _comments ?? [] };
  };

  /**
   * Comment-only fast path. Used by `getAllComments` and any path that
   * needs comments without the full token stream — most notably
   * `applyDisableDirectives` and rules like `ban-ts-comment` which only
   * read `sourceCode.getAllComments()`.
   *
   * If `parsedComments` was supplied at construction (the common case
   * when SourceCode is built from a fresh oxc-parser run), this avoids
   * the full text-scan tokenizer entirely. Otherwise we fall back to
   * `ensureTokens` which populates `_comments` as a side effect.
   */
  const ensureComments = (): Comment[] => {
    if (_comments == null) {
      if (input.parsedComments) {
        _comments = buildCommentsFromParsed(
          input.parsedComments,
          text,
          getLso(),
        );
      } else {
  ...
  const sc: SourceCode = {
    ...
    // Full ESTree + TypeScript visitor-keys map (from
    // `@typescript-eslint/visitor-keys`, see `visitor-keys.ts`). The
    // runner needs the TS-aware set because plugin rules iterate
    // `sourceCode.visitorKeys[node.type]` on .ts files and expect to
    // recurse into TSEnumDeclaration / TSInterfaceDeclaration /
    // TSAsExpression children. The plain ESTree KEYS would return
    // `undefined` for those types, silently skipping every TS
    // subtree.
