...
/**
 * Pure helpers for `source-code.ts` — extracted to keep the factory
 * file under control. Everything here is stateless: scope helpers
 * take the ScopeManager (and AST root, where needed) as arguments;
 * token-option normalizers take the user's options struct.
 *
 * Why a separate file: each fix on these helpers ships with several
 * paragraphs of comments documenting the ESLint behavior they match.
 * Inlining all of that into the SourceCode factory pushed it past
 * 1000 lines, blurring the line between "API surface" and "internal
 * algorithm". Splitting lets the factory focus on wiring the public
 * `SourceCode` object together.
 */
