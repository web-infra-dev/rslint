...
/**
 * Resolve the worker entry file (lint-worker.js).
 *
 * In production: import.meta.url is `dist/worker-pool.js`, so siblings
 * resolve to `dist/lint-worker.js` directly.
 *
 * In tests / dev: import.meta.url is `src/worker-pool.ts`, but
 * worker_threads requires a `.js` file (it can't run TypeScript directly).
 * We map the path from `src/` → `dist/` so test runs naturally pick up the
 * built artifact (callers must have run `pnpm build` once before testing).
 */
const WORKER_FILE = (() => {
  let p = fileURLToPath(new URL(import.meta.url));
  // Map src→dist if present
  p = p.replace(
    /[/\\]src[/\\]([^/\\]+)\.ts$/,
    (_m, base) => `/dist/${base}.js`,
  );
  // If we're already in dist or post-mapping, ensure the file is .js
  return resolve(dirname(p), 'lint-worker.js');
})();
...
export class WorkerPool {
  ...
  private async spawnWorker(id: number): Promise<WorkerSlot> {
    return new Promise((resolveOk, reject) => {
      ...
      let worker: Worker;
      try {
        worker = new Worker(WORKER_FILE, workerOptions);
      } catch (err) {
        reject(err as Error);
        return;
      }
