...
/**
 * Bidirectional IPC client over a Node Duplex pair (typically the runner
 * process's stdin/stdout, with a Go peer on the other side).
 *
 * Wire format and message shape mirror Go's `internal/api.BidirectionalService`:
 *
 *   `[4 bytes u32 LE length][JSON payload]`
 *   payload  = { kind: MessageKind, id: number, data: unknown }
 *
 * This is the Node-side counterpart to Go's BidirectionalService. The two
 * are deliberately not import-coupled — keeping the two packages
 * independent lets the IPC contract evolve via tests rather than via a
 * shared type module. The contract tests in `internal/api` and the
 * Node-side process-spawn tests in `packages/rslint/tests/binary-ipc-*`
 * together pin the wire format.
 *
 * Concurrency model:
 *
...
/**
 * Per-frame body cap, matched to Go's `internal/api.maxFrameSize`.
...
export class IpcClient {
  ...
  private readonly onChunk = (chunk: Buffer): void => {
    ...
    while (this.bufferedBytes >= HEADER_BYTES) {
      ...
      // internal/api/bidirectional.go. A frame length that exceeds the
      ...
      try {
        ...
        process.stderr.write(
          `[ipc-client] malformed JSON in frame (len=${len}): ${(err as Error).message}\n`,
        );
        continue;
      }
  ...
  private readonly onStreamError = (err: Error): void => {
    process.stderr.write(`[ipc-client] stream error: ${err.message}\n`);
    this.onEnd();
  };
  ...
  private routeResponse(msg: IpcMessage): void {
    ...
    if (!p) {
      process.stderr.write(
        `[ipc-client] orphan response id=${msg.id} kind=${msg.kind}\n`,
      );
      return;
    }
  ...
  private dispatchNotification(msg: IpcMessage): void {
    ...
    if (!handler) {
      // Unhandled notifications are common during early bring-up; warn
      // once-ish to stderr but don't error.
      process.stderr.write(
        `[ipc-client] unhandled notification kind=${msg.kind}\n`,
      );
      return;
    }
...
async function runSafely(fn: () => unknown, tag: string): Promise<void> {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[ipc-client] handler ${tag} threw: ${message}\n`);
  }
}
