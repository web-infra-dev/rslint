 ...
 * runner Worker. Surface tracks **ESLint v10** (empirically pinned
 * against eslint@10.x) — members removed in v10 (`getJSDocComment`,
 ...
 *     after the AST visit.
 *
 * The heavier sub-modules live next door (split for maintainability —
 * `context.ts` was a 1400-line single file before):
 *
...
export interface CreateContextOptions {
   ...
   */
  lsoCache?: number[];
  /** Normalized ESTree AST root. */
  ast: ESTreeNode;
  /** Lazy scope-manager factory (computed on first scope-related call). */
  scopeManagerFactory: () => unknown;
  /**
   * Optional: a SourceCode shared across every rule for this file. When
   * provided, `createRuleContext` skips building its own SourceCode and
   * wires this one onto `ctx.sourceCode`. Sharing the instance lets all
   * rules + `applyDisableDirectives` hit the same lazy caches (tokens,
   * comments, scopeManager), so e.g. one rule's `getAllComments` call
   * primes every other rule's. Without sharing, each rule pays the
   * lazy-init cost independently — see `lintFile` for the construction
   * site. The `text`/`ast`/`scopeManagerFactory` fields above are still
   * honored as the fallback when this is omitted (in-process tests).
   */
  sourceCode?: SourceCode;
  /**
   * Forwarded oxc-parser-emitted comments. Only consulted when
   * `sourceCode` above is NOT supplied (i.e. createRuleContext builds
   * its own SourceCode). See `SourceCodeBuildInput.parsedComments` for
   * the rationale.
   */
...
export function createRuleContext(opts: CreateContextOptions): RuleContext {
  ...
  // Shared-SourceCode fast path: lintFile builds one SourceCode per file
  // and passes the same instance to every rule's createRuleContext. When
  // present we reuse it directly; the lazy state (tokens, comments,
  // scopeManager) is then primed by whichever rule first touches each
  // API and reused by the rest.
  const sourceCode =
    opts.sourceCode ??
    createSourceCode({
      text,
      ast,
      scopeManagerFactory,
      parsedComments: opts.parsedComments,
    });
