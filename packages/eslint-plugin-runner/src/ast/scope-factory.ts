 ...
 * `getScope()` cost nothing.
 *
 * Implementation note: we don't import the analyzers at the top of the
 * file because both have nontrivial init cost; we want them to be loaded
 * only on the first call site of the factory return value, which is
 * already the lazy path the SourceCode builds.
 */
...
function analyze(ast: object, opts: ScopeFactoryOptions): unknown {
  ...
  if (useTs) {
    // Lazy ESM import in a sync context isn't possible; we do require()
    // via createRequire below. Both bundled analyzers ship CJS by default.
    // eslint-disable-next-line @typescript-eslint/no-var-requires -- ESM module loading a CJS-only package (@typescript-eslint/scope-manager ships CJS); createRequire is the standard ESM↔CJS bridge.
    const tsScope = require('@typescript-eslint/scope-manager') as {
  ...
  // eslint-disable-next-line @typescript-eslint/no-var-requires -- ESM module loading a CJS-only package (eslint-scope ships CJS); createRequire is the standard ESM↔CJS bridge.
  const eslintScope = require('eslint-scope') as {
  ...
  // We pass oxc-parser's own `visitorKeys` (the same table the runner
  ...
  return eslintScope.analyze(ast, {
    ...
    fallback: 'iteration',
    childVisitorKeys: oxcVisitorKeys as Record<string, readonly string[]>,
    // globals are seeded post-analysis on most ESLint versions; we do it
    // here too, for parity. The TS analyzer doesn't accept globals at
    // construct time so we leave that case to seedGlobals below.
    ...(opts.globals ? {} : {}),
  });
}
...
export function seedEcmaGlobals(scopeManager: unknown): void {
  ...
  for (const name of Object.keys(BUILTIN_GLOBAL_NAMES)) {
...
// Lazy load: the `globals` package is pure data and a single property
// access on `.builtin`. Cached on first use.
let _builtinNames: Record<string, unknown> | null = null;
const BUILTIN_GLOBAL_NAMES = new Proxy(
  {},
  {
    ownKeys() {
      if (!_builtinNames) {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const g = require('globals') as { builtin?: Record<string, unknown> };
        _builtinNames = g.builtin ?? {};
      }
      return Object.keys(_builtinNames);
    },
...
// `require` is needed for the lazy CJS imports above; ts-go's tsgo build
// supports CJS require in ESM files via this pattern.
