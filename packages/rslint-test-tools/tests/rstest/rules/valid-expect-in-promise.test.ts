...
ruleTester.run('valid-expect-in-promise', {} as never, {
  valid: [
    ...
    {
      code: `test('case', async () => {
        let pending = fallback || load().then(value => assert.equal(value, 1));
        await pending;
      });`,
    },
    {
      code: `test('case', async () => {
        let pending = condition ? load().then(value => assert.equal(value, 1)) : other;
        await pending;
      });`,
    },
    {
  ...
  invalid: [
    {
      code: `test('case', () => {
        list.push(load().then(value => assert.equal(value, 1)));
      });`,
    ...
    {
      code: `test('case', async () => {
        let pending = load().then(value => assert.equal(value, 1));
