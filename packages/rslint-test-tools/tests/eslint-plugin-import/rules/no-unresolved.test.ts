// Upstream: https://github.com/import-js/eslint-plugin-import/blob/v2.32.0/tests/src/rules/no-unresolved.js
import fs from 'node:fs';
import path from 'node:path';
import {
  RuleTester as ImportRuleTester,
  type ValidTestCase,
  type InvalidTestCase,
} from '../rule-tester.js';

// Keep fixtures below the wrapper's working directory for upstream's
// caseSensitiveStrict tests, which deliberately change only the cwd spelling.
const root = fs.mkdtempSync(path.join(process.cwd(), '.no-unresolved-'));
const archive = fs.readFileSync(
  path.resolve(
    import.meta.dirname,
    '../../../../../internal/plugins/import/rules/no_unresolved/testdata/upstream.txtar',
  ),
  'utf8',
);
...
const CASE_SENSITIVE_FS = !fs.existsSync(path.join(root, 'BAR.js'));
const parsers = { BABEL_OLD: 'babel', TS_OLD: 'old' };
const getTSParsers = () => ['typescript'];
const testVersion = <T>(_: string, run: () => T): T => run();
const test = <T extends ValidTestCase>(item: T): T => item;
const rule = null as never;
const ruleTester = {
  run(name: string, _rule: never, cases: { valid: any[]; invalid: any[] }) {
    if (
      name.includes('webpack') ||
      [...cases.valid, ...cases.invalid].some(
        (item) => item.settings?.['import/resolver'] === 'webpack',
      )
    ) {
      it.skip(`${name}: JavaScript resolver plugins are not supported`, () => {});
      return;
    }
    ...
    describe(name, () => {
      ...
      for (const item of [...cases.valid, ...cases.invalid].filter(
        ...
        it.skip(`Babel export-default-from: ${item.code}`, () => {});
      }
      new ImportRuleTester().run('no-unresolved', null as never, {
        valid: cases.valid.filter(supported).map(adapt) as ValidTestCase[],
        invalid: cases.invalid
          .filter(supported)
          .map(adapt) as InvalidTestCase[],
      });
    });
  },
};
...
function runResolverTests(resolver: string) {
  ...
  if (!CASE_SENSITIVE_FS) {
    ...
    const relativePath = path.relative(
      cwd,
      path.join(root, 'jsx/MyUnCoolComponent.jsx'),
    );
