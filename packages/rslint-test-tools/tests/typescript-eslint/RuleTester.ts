...
function checkDiagnosticEqual(rslintDiagnostic: Diagnostic[], tsDiagnostic: TsDiagnostic[]) {
  assert(rslintDiagnostic.length === tsDiagnostic.length, `Length mismatch: ${rslintDiagnostic.length} !== ${tsDiagnostic.length}`);
  for (let i = 0; i < rslintDiagnostic.length; i++) {
    ...
    if(tsDiag.endLine) {
      assert(rslintDiag.range.end.line === tsDiag.endLine, `End line mismatch: ${rslintDiag.range.end.line} !== ${tsDiag.endLine}`);
    }
    if(tsDiag.column) {
      assert(rslintDiag.range.start.column === tsDiag.column, `Start column mismatch: ${rslintDiag.range.start.column} !== ${tsDiag.column}`);
    }
    if(tsDiag.endColumn) {
      assert(rslintDiag.range.end.column === tsDiag.endColumn, `End column mismatch: ${rslintDiag.range.end.column} !== ${tsDiag.endColumn}`);
    }
  }
...
export class RuleTester {
  ...
  public run(ruleName:string, cases: {
    valid: string[],
    invalid: {
      code: string,
      errors: any[]
    ...
    test(ruleName, async () => {
      ...
      const tsconfig = path.resolve(import.meta.dirname, './fixtures/tsconfig.virtual.json')
      ...
      await test('valid', async () => {
        for (const code of cases.valid) {
         ...
         assert(diags.diagnostics?.length === 0, `Expected no diagnostics for valid case, but got: ${JSON.stringify(diags)}`);
        }
      });
      await test('invalid', async (t) => {
         ...
         assert(diags.diagnostics?.length >0, `Expected diagnostics for invalid case`);
