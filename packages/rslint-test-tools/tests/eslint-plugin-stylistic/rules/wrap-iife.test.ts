 ...
 *   packages/eslint-plugin/rules/wrap-iife/wrap-iife.test.ts
 *
 * Transformations applied per the porting spec:
 *  - `run({ name, rule, valid, invalid })` -> `ruleTester.run('wrap-iife', null as never, { valid, invalid })`
 *    (the `name` / `rule` / `#test` imports are dropped).
 *  - The three local error helpers (`wrapInvocationError` / `wrapExpressionError`
 *    / `moveInvocationError`) are kept as plain `{ messageId }` constants — each
 *    is already the fully-evaluated error object the RuleTester consumes.
 *  - `parserOptions: { ecmaVersion: 2020 }` is dropped from the 12 cases that
 *    carried it (dynamic `import(...)` and optional-chaining). rslint resolves
 *    via tsconfig (target esnext), so both parse without it — verified below.
 *  - No `$` unindent tag, no `type` fields, and no suggestions exist upstream.
 *
 ...
 *  - All 86 valid cases report 0 diagnostics. This includes the `with (...)`
 *    statements (sloppy-mode syntax) and the dynamic-`import(...)` cases — ts-go
 *    parses all of them without a syntax error, so none needed isolation.
 *  - All 42 invalid cases match upstream on diagnostic count (every case pins
 *    exactly one error), message, and autofix `output` (including the
 *    comment-preserving and optional-chaining fixers). Upstream pins no
 *    line/column for this rule, so positions are not asserted.
...
const wrapInvocationError = { messageId: 'wrapInvocation' };
const wrapExpressionError = { messageId: 'wrapExpression' };
const moveInvocationError = { messageId: 'moveInvocation' };

ruleTester.run('wrap-iife', null as never, {
  valid: [
    {
      code: '(function(){ }());',
      options: ['any'],
