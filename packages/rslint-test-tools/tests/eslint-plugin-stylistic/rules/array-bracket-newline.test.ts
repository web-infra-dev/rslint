 ...
 *   packages/eslint-plugin/rules/array-bracket-newline/array-bracket-newline.test.ts
 *
 * Transformations applied per the porting spec:
 *  - `run({ name, rule, valid, invalid })` -> `ruleTester.run('array-bracket-newline', null as never, { valid, invalid })`
 ...
 * Every invalid case pins an explicit `errors` array — there are NO output-only
 * cases. The `._css_` / `._json_` / `._markdown_` test files don't exist for this
 * rule.
 *
 * All fixtures are plain ECMAScript (ArrayExpression / ArrayPattern) that the
 * ts-go parser accepts under strict/module semantics, so nothing is isolated to
 * KNOWN GAPS. There are NO known gaps for this rule.
 */
...
ruleTester.run('array-bracket-newline', null as never, {
  valid: [
    /**
     * ArrayExpression
