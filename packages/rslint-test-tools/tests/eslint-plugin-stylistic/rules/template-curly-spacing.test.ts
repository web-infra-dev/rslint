 ...
 *   packages/eslint-plugin/rules/template-curly-spacing/template-curly-spacing.test.ts
 *
 * Transformations applied per the porting spec:
 *  - `run({ name, rule, ... })` -> `ruleTester.run('template-curly-spacing', null as never, { valid, invalid })`
 *  - Each `errors[]` entry keeps its `messageId` and the `line/column/endLine/endColumn`
 *    that upstream pins. None of the four messages
 *    (`expectedBefore`/`expectedAfter`/`unexpectedBefore`/`unexpectedAfter`) use
 *    `{{}}` interpolation, so the RuleTester resolves each `messageId` to its full
 *    static message text from the plugin's meta. The rule sets an explicit `loc`
 *    per report — `${`/`}` token edge for `unexpectedAfter`/`unexpectedBefore`,
 *    the two-char `${`/one-char `}` span for `expectedAfter`/`expectedBefore` — and
 *    the RuleTester asserts the diagnostic `range.start`/`range.end` against those
 *    coordinates.
 *  - Every invalid case also pins `output`; the RuleTester asserts the `--fix`
 *    output equals it.
 *
 * The upstream test file is a single `run()` block (no `if (!skipBabel)` block,
 * no Babel/Flow cases, no `$` unindent, no spread/helpers). Every fixture is a
 * single-quoted JS string whose contents are template-literal source text;
 * several embed real newlines (`\n`) inside block comments so endLine/endColumn
 * cross lines. All code is valid TS and parses under ts-go (template literals,
 * tagged templates, comments inside `${}` — no octal/escape/import-attribute
 * fixtures), so nothing trips ts-go's strict/module parser. The `._js_` /
 * `._ts_` / `._css_` / `._json_` / `._markdown_` split test files don't exist for
 * this rule (it ships a single `template-curly-spacing.test.ts`).
 *
...
ruleTester.run('template-curly-spacing', null as never, {
  valid: [
    '{ foo }',
    '`${foo} ${bar}`',
    { code: '`${foo} ${bar} ${\n  baz\n}`', options: ['never'] },
