 ...
 *   packages/eslint-plugin/rules/space-in-parens/space-in-parens.test.ts
 *
 * Transformations applied per the porting spec:
 *  - `run({ name, rule, lang, ... })` -> `ruleTester.run('space-in-parens', null as never, { valid, invalid })`
 *  - `parserOptions` (ecmaVersion) dropped — rslint always parses at esnext.
 *  - The rule's messageIds (`missingOpeningSpace` / `missingClosingSpace` /
 *    `rejectedOpeningSpace` / `rejectedClosingSpace`) take no `data`, so they map
 ...
 * no Babel/Flow cases, no `$` unindent (every fixture is a single-line string or
 * uses `\n` escapes), and no `readFileSync` external fixtures.
 *
...
ruleTester.run('space-in-parens', null as never, {
  valid: [
