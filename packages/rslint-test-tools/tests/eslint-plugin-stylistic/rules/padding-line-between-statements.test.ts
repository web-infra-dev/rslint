 ...
 *  - `run({ name, rule, lang, valid, invalid })` -> `ruleTester.run('padding-line-between-statements', null as never, { valid, invalid })`.
 *  - The `$` unindent template tag (from eslint-vitest-rule-tester) is fully
 *    evaluated to its real multi-line string and re-emitted as a single-quoted
 *    `\n`-escaped literal (the established convention in this directory — see
 *    quotes.test.ts / brace-style.test.ts), byte-for-byte the upstream value.
 *  - The two `...['using','await using'].flatMap(...)` spreads in each of the
 *    upstream valid/invalid arrays are evaluated and expanded to their final cases.
 *  - `parserOptions` (ecmaVersion / sourceType / ecmaFeatures) dropped — rslint
 *    resolves via tsconfig; the RuleTester picks a `.tsx` fixture when JSX is present.
 *
 * The upstream messages carry no `data` interpolation (static strings):
 *   unexpectedBlankLine: 'Unexpected blank line before this statement.'
 *   expectedBlankLine:   'Expected blank line before this statement.'
 * Upstream pins no line/column/endLine/endColumn and no suggestions, so neither
 * is asserted. No `._css_`/`._json_`/`._markdown_` test files exist for this rule.
 *
...
ruleTester.run('padding-line-between-statements', null as never, {
  valid: [
    // ---- from padding-line-between-statements._js_.test.ts ----
      ...
      options: [
        {
          ...
          prev: '*',
          next: {
            selector: 'FunctionDeclaration[id.name="bar"]',
          },
        },
      ],
    },
      ...
      options: [
        {
          blankLine: 'always',
          prev: {
            selector: 'ExpressionStatement[expression.callee.name="foo"]',
          },
          next: '*',
        },
      ],
    ...
    // ---- from padding-line-between-statements._ts_.test.ts ----
    {
      code: 'module.exports=1',
      options: [
        {
  ...
  invalid: [
      ...
      options: [
        {
          ...
          next: {
            selector: 'ExpressionStatement[expression.callee.name="bar"]',
          },
        },
      ],
    ...
    {
      code: 'await using a = x\n\nfoo()',
      output: 'await using a = x\nfoo()',
      options: [
        {
          blankLine: 'never',
          prev: 'using',
