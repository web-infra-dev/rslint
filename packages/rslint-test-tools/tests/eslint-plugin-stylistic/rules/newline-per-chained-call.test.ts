 ...
 *   packages/eslint-plugin/rules/newline-per-chained-call/newline-per-chained-call.test.ts
 *
 * Transformations applied per the porting spec:
 *  - `run({ name, rule, ... })` -> `ruleTester.run('newline-per-chained-call', null as never, { valid, invalid })`
 *  - Upstream `code`/`output` written as `[ '...', '...' ].join('\n')` are
 *    evaluated to their final multi-line string literals (verbatim contents).
 *  - `parserOptions: { ecmaVersion: 2020 }` (on the optional-chaining cases)
 *    dropped — rslint parses optional chaining as valid TS regardless.
 *
 ...
 * this rule. One fixture embeds a raw U+2029 (PARAGRAPH SEPARATOR) line
 * terminator (`['foo' +   + 'bar']`); ts-go counts it as a line break
 * exactly like ESLint's parser (verified: the diagnostic spans `endLine: 2`),
 * so it stays in the green set. Nothing was isolated into KNOWN GAPS.
 */
...
ruleTester.run('newline-per-chained-call', null as never, {
  valid: [
    '_\n.chain({})\n.map(foo)\n.filter(bar)\n.value();',
    'a.b.c.d.e.f',
    'a()\n.b()\n.c\n.e',
