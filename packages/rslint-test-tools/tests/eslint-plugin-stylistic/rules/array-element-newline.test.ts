 ...
 *  - `run({ name, rule, valid, invalid })` -> `ruleTester.run('array-element-newline', null as never, { valid, invalid })`
 *  - `parserOptions: { ecmaVersion: 6 }` dropped — rslint always parses at esnext,
 *    where array destructuring (ArrayPattern) is supported, so these stay green.
 *
 ...
 * It imports only `run` (no `$` unindent tag), so every `code` string is a plain
 * template literal copied verbatim. Every invalid case pins an explicit `errors`
 * array — there are NO output-only cases. The `._css_` / `._json_` /
 * `._markdown_` test files don't exist for this rule.
 *
 * All fixtures are plain ECMAScript (ArrayExpression / ArrayPattern, including
 * sparse arrays with holes) that the ts-go parser accepts under strict/module
 * semantics, so nothing is isolated to KNOWN GAPS. There are NO known gaps for
 * this rule.
 */
...
ruleTester.run('array-element-newline', null as never, {
  valid: [
    ...
    // "never"
    { code: 'var [a,[b,c]] = foo;', options: ['never'] },

    // { minItems: 3 }
    { code: 'var [] = foo;', options: [{ minItems: 3 }] },
    { code: 'var [a] = foo;', options: [{ minItems: 3 }] },
    { code: 'var [a, b] = foo;', options: [{ minItems: 3 }] },
    { code: 'var [a,\nb,\nc] = foo;', options: [{ minItems: 3 }] },

    /**
     * ArrayExpression & ArrayPattern
     * { ArrayExpression: "always", ArrayPattern: "never" }
     */
    { code: 'var [a, b] = [1,\n2]', options: [{ ArrayExpression: 'always', ArrayPattern: 'never' }] },
  ],
