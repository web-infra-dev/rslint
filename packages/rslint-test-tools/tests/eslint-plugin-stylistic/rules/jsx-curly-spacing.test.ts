/**
 * @fileoverview Tests for jsx-curly-spacing rule.
 *
 * Ported verbatim from @stylistic/eslint-plugin v5.10.0:
 *   packages/eslint-plugin/rules/jsx-curly-spacing/jsx-curly-spacing.test.ts
 *
 * Transformations applied per the porting spec:
 *  - `run({ name, rule, parserOptions, valid: valids(...), invalid: invalids(...) })`
 *    -> `ruleTester.run('jsx-curly-spacing', null as never, { valid: [...], invalid: [...] })`.
 ...
 *    `@typescript-eslint/parser` parsers, appending a `// parser: ...` comment to
 *    the code. That is test-harness plumbing — the per-case `code`/`output`/`errors`
 *    objects below are the verbatim source cases. rslint runs on ts-go (the TS-parser
 *    semantics upstream verifies), and the appended comment is inert for a JSX rule.
 *    (Upstream's own CI also skips the babel variant: `skipBabel = ESLint.version >= 10`,
 *    and the installed ESLint here is 10.5.0.)
 *  - `parserOptions` (ecmaFeatures.jsx) dropped — rslint resolves via tsconfig and the
 *    RuleTester routes JSX fixtures to `.tsx`.
 *  - `features: ['fragment']` dropped from the two `<>...</>` cases: it only gates which
 *    upstream parsers run; ts-go parses JSX fragments natively.
 *  - `name` / `rule` / imports dropped.
 *
 * The upstream cases use no `$`/unindent tag (all plain backtick or single-quote
 * literals, preserved byte-for-byte), no `suggestions`, and no external-fixture
 * (`readFileSync`) cases. The `._css_` / `._json_` / `._markdown_` test files don't
 * exist for this rule. No case surfaced an rslint<->upstream gap, so there is no
 * KNOWN GAPS section.
 */
