...
- It checks the test inclusion and skip status from `packages/rslint-test-tools/rstack.config.mts` and test files under `packages/rslint-test-tools/tests/typescript-eslint/rules/`.
...
- If a rule is not included in the `include` list of `rstack.config.mts`, its status is `partial-test`.
