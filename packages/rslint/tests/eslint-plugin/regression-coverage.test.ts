 ...
 * surfaces across the runner (WorkerPool / SourceCode / token-builder /
...
// Tokenize via the real path (native parser → columnar token stream → token-builder),
// replacing the deleted hand-written JS tokenizer. `script` (sloppy) matches the sloppy-mode
// token tests below and the espree oracle (`eTok`, which uses sourceType: 'script').
function lexTokens(src: string) {
  const parsed = nativeParse('test.js', src, 'script', false);
  return buildTokens(
    parsed.tokenTypes,
    parsed.tokenStarts,
    parsed.tokenEnds,
...
describe('tokenizer treats `/` after identifier `from`/`of`/`as`/`async` as division', () => {
  ...
  test('`await /re/g.test(x)` — `await` is Identifier token, `/re/g` is a RegExp (espree.parse)', () => {
    // Verified against espree.parse (the ESLint token contract): in an async function `await`
    // is an Identifier-TYPE token, and `/re/g` is its operand — a single RegularExpression
    // token (NOT division). The deleted JS tokenizer wrongly decomposed it; oxc matches espree.
    const src = 'async function f() { return await /re/g.test(x); }';
    const tokens = lexTokens(src);
    expect(tokens.find((t) => t.value === 'await')?.type).toBe('Identifier');
    expect(tokens.find((t) => t.type === 'RegularExpression')?.value).toBe(
      '/re/g',
    );
    expect(
...
describe('same esquery selector registered by 2 rules deduplicates to 1 bucket entry', () => {
  test('two rules with `:matches(Identifier)` produce one entry with 2 listeners', async () => {
    const { mergeListeners } =
      await import('../../src/eslint-plugin/linter/listener-merge.js');
...
// (removed) "unterminated regex at EOL" — it exercised the deleted JS tokenizer's recovery
// fallback. espree.parse and oxc both treat `/abc\n` as a parse error (no tokens emitted), so
// there is nothing to assert through the native token path.
...
function rTok(src: string): TT[] {
  return lexTokens(src).map((t) => ({
    type: t.type,
    value: t.value,
  }));
}
function eTok(src: string): TT[] {
  // espree.parse tokens — the ESLint token contract rules actually consume. NOT
  // espree.tokenize, whose pure-lexer output mis-classifies reserved words in member/property
  // position (e.g. `obj.class` → Keyword instead of Identifier), so it's the wrong oracle.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return ast.tokens.map((t: TT) => ({ type: t.type, value: t.value }));
}
...
describe('tokenizer: strict future-reserved words are Identifier (espree@11)', () => {
  ...
  test('reserved words in member position are Identifier, except static/let/yield (espree.parse)', () => {
    // espree.parse: a reserved word as a member property is an Identifier token; only
    // static/let/yield stay Keyword there. (espree.tokenize disagrees — it's the wrong oracle.)
    for (const [src, word, type] of [
      ['obj.class;', 'class', 'Identifier'],
      ['obj.delete;', 'delete', 'Identifier'],
      ['obj.if;', 'if', 'Identifier'],
      ['obj.static;', 'static', 'Keyword'],
    ] as const) {
      expect(rTok(src)).toEqual(eTok(src));
      expect(rTok(src).find((t) => t.value === word)?.type).toBe(type);
    }
  });
});
...
describe('normalize-ast: TemplateElement.range includes delimiters (espree@11)', () => {
  ...
  function runnerTplRanges(
    ...
    const parsed = nativeParse(`t.${ext}`, src, 'module', ext === 'tsx');
