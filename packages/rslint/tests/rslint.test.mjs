...
describe('Rslint class', () => {
  ...
  test('remaining multi-edit fix is merged against final output generation', async () => {
    ...
    await writeFile(
      ...
      `export default {
				rules: {
					block: {
						meta: { fixable: 'code', schema: [], messages: { block: 'block' } },
						create(context) {
							return { Program(node) {
								context.report({
									node,
    ...
    await writeFile(
      ...
      `import blocking from ${JSON.stringify(pathToFileURL(pluginPath).href)};
			export default [{
				plugins: { blocking },
				rules: { 'blocking/block': 'error', 'no-else-return': 'error' },
			}];\n`,
    );
...
function f(value) {
  if (value) {
    ...
    try {
      ...
      const applied =
        finalSource.slice(0, fix.range[0]) +
        fix.text +
        finalSource.slice(fix.range[1]);
      expect(applied).toBe(expected.output);
    } finally {
  ...
  test('community plugin BOM edits survive the complete API fix path', async () => {
    const tmp = await mkdtemp(path.join(os.tmpdir(), 'rslint-api-plugin-bom-'));
    const pluginPath = path.join(tmp, 'bom-plugin.mjs');
    await writeFile(
      pluginPath,
      `export default {
				rules: {
					remove: {
						...
						create(context) {
							return { Program(node) {
								if (!context.sourceCode.hasBOM) return;
								context.report({
									node,
									messageId: 'remove',
    ...
    await writeFile(
      path.join(tmp, 'rslint.config.mjs'),
      `import bom from ${JSON.stringify(pathToFileURL(pluginPath).href)};
			export default [{ plugins: { bom }, rules: { 'bom/remove': 'error' } }];\n`,
    );

    const inspector = new Rslint({ cwd: tmp });
    const fixer = new Rslint({ cwd: tmp, fix: true });
    const BOM = String.fromCharCode(0xfeff);
    const source = BOM + 'const value = 1;\n';
    try {
      const [before] = await inspector.lintText(source, {
        filePath: 'probe.js',
      });
      expect(before.output).toBeUndefined();
      expect(before.fixableErrorCount).toBe(1);
      expect(before.messages).toHaveLength(1);
      expect(before.messages[0].fix).toEqual({ range: [-1, 0], text: '' });
      expect(before.messages[0].suggestions?.[0].fix).toEqual({
        range: [-1, 0],
        text: '',
      });

      const [after] = await fixer.lintText(source, { filePath: 'probe.js' });
      expect(after.output).toBe('const value = 1;\n');
