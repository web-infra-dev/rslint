...
describe('Rslint class', () => {
  ...
  test.each([
    { discovery: false, fix: false },
    { discovery: false, fix: true },
    { discovery: true, fix: false },
    { discovery: true, fix: true },
  ])(
    'lintText preserves unnamed input through config and fix rounds (%j)',
    async ({ discovery, fix }) => {
      const cwd = await mkdtemp(path.join(os.tmpdir(), 'rslint-unnamed-'));
      const code = 'let values: Array<string> = [];';
      const fixed = 'let values: string[] = [];';
      const config = [
        {
      ...
      await writeFile(
        ...
        JSON.stringify({ bin: 'cli.js', files: [] }),
      );
      await writeFile(
        path.join(cwd, 'rslint.config.mjs'),
        `export default ${JSON.stringify(config)};`,
      );
      // Even an existing file with the internal fallback name must not turn an
      // unnamed buffer into a package executable. Explicitly naming it must work.
      await writeFile(path.join(cwd, '__text__.ts'), code);
      const rslint = new Rslint({
        cwd,
        overrideConfigFile: discovery ? undefined : true,
        overrideConfig: discovery ? undefined : config,
        fix,
      });
      try {
        const checkUnnamed = async () => {
          ...
          expect(results[0].output).toBe(fix ? fixed : undefined);
        };
        await checkUnnamed();
        for (const filePath of ['__text__.ts', 'named.ts']) {
          const [named] = await rslint.lintText(code, { filePath });
          const ignored = named.messages.filter(
            (message) => message.ruleId === 'node/no-unpublished-bin',
          );
          expect(ignored).toHaveLength(1);
          expect(ignored[0].messageId).toBe('invalidIgnored');
          expect(ignored[0].message).toBe(
            "npm ignores 'cli.js'. Check 'files' field of 'package.json' or '.npmignore'.",
          );
          ...
          expect(
            named.messages.some(
              (message) => message.ruleId === 'node/hashbang',
            ),
          ).toBe(!fix);
        }
        await checkUnnamed();
        const [disk] = await rslint.lintFiles(['__text__.ts']);
        expect(
          disk.messages.some(
            (message) => message.ruleId === 'node/no-unpublished-bin',
          ),
