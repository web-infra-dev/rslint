...
/**
 * Compute a rule's runtime `context.options` from user options +
 * `rule.meta.defaultOptions`, matching ESLint v10's two passes:
 *   1. deep-merge `meta.defaultOptions` under the user array (`deepMergeArrays`);
 *   2. fill schema `properties[k].default`s in place (ajv `useDefaults`).
 *
 * ajv only fills properties of an object already occupying a slot — it never
 * creates a positional slot, so a severity-only config (no options array)
 * never fabricates an option object. A slot-level `default` (outside
 * `properties`) is likewise never materialized; `meta.defaultOptions` wins.
 */
