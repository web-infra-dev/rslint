 ...
 *
 * Source of truth: the vendored `VISITOR_KEYS_TABLE` (see
 * `visitor-keys-table.ts`) — oxc-parser 0.133's `visitorKeys`, captured as
 * a hardcoded table so removing the npm `oxc-parser` dep keeps it. Since
 * the runner walks the AST oxc PRODUCES, this table is authoritative by
 * construction: it covers exactly the node types oxc can emit (JS + JSX +
 * TS; 165 types).
 *
 * Verified (see `tests/ast/visitor-keys.test.ts`, a drift guard):
 *   - byte-identical to `@typescript-eslint/visitor-keys` for every shared
 *     node type (159 types, 0 order/content mismatch), so TS traversal
 *     order stays exactly what `@typescript-eslint/parser` declares;
 *   - its JS-node projection equals `eslint-visitor-keys`, so plain-JS
 *     traversal order stays exactly what ESLint core declares;
 *   - complete for oxc's output (no emitted node type is missing from the
 *     table), so the `getVisitorKeys` fallback is effectively unreachable
 *     in production.
 * The guard re-runs those comparisons against the canonical ESLint keys so
 * a future re-vendor that drifts from ESLint fails CI loudly.
 *
 ...
 */
import { VISITOR_KEYS_TABLE } from './visitor-keys-table.js';

const _vkObj = Object.create(null) as Record<string, readonly string[]>;
for (const k of Object.keys(VISITOR_KEYS_TABLE)) {
  _vkObj[k] = VISITOR_KEYS_TABLE[k];
}
