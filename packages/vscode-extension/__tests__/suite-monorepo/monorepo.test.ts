...
`;
  ...
  test('creating new sub-package config for bar should override root config', async () => {
    const doc = await openFile('packages/bar/src/index.ts');
    const editor = await vscode.window.showTextDocument(doc);
    ...
    // 2. Create a new config for bar with opposite rules
    const barConfigPath = path.join(
      getWorkspaceRoot(),
      'packages/bar/rslint.config.js',
    );

    const barConfig = `export default [
  {
    languageOptions: {
      parserOptions: {
        projectService: false,
...
`;
  ...
  test('deleting root config should not affect sub-package with its own config', async () => {
