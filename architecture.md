...
| `internal/config/gitignore/`   | Git ignore pattern matching, config-scoped collection, and directory reachability                                              | Staged JS/TS catalogs carry a filesystem-independent cursor through their existing walk, pruning Git-inaccessible subtrees and freezing observed patterns for lint-target admission; low-level pre-resolved configs and the LSP workspace fallback reuse the direct collector. Rules can use `NewMatcher` for explicit patterns without config discovery or target-admission policy                                                         |
