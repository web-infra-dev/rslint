...
- **SourceFile-Owned Module Syntax Reuse**: the resident-capable LSP diagnostics path may attach the syntax-only module-specifier projection to an immutable ts-go `SourceFile`. Programs reusing that exact file object share collection, while module resolution remains local to each `Program`. Replaced files and files removed from a Program carry the attached data out with their own AST lifetime; no LSP Server map, project-membership sweep, or explicit reset owns this cache. A fresh fallback in this path can retain data only for its transient SourceFile lifetime. Speculative LSP Programs, LSP paths without resident support, CLI, and API runs keep collection run-local.
