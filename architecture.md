...
| `internal/program/loader/`     | Shared project selection and invocation-scoped Program construction                                                            | Owns project selection, extension eligibility and exact/frozen source membership for CLI, API and LSP. Adapters supply parsed metadata and Programs; the selector never adds targets or owns editor caches. CLI/API also use its request-scoped construction and binding; LSP retains Session, overlay and watcher lifetimes                                                                                                                |
...
2. **Program Loading**: `internal/config/target` freezes targets, their governing config owners, and path identities without consulting Programs. `config.FileConfigResolver` matches and merges each file's configuration once. `config.ResolveProjectPolicy` projects the effective explicit project declaration and service/root/reset options from that same immutable result; it performs no matching, filesystem lookup, or project construction. CLI/API use `config/lint.Resolver` both before construction and after source binding, sharing its file and matched-shape caches. Before construction, file configuration resolves in parallel with at most `GOMAXPROCS` workers; policy projection and failures retain target input order. `--singleThreaded` keeps this preparation serial. Workers finish before the request proceeds, including when a file fails. LSP consumes its existing document snapshot's resolved config. Node only evaluates and serializes values; presets remain ordinary reusable values.
   ...
   `program/loader.Session` assembles one project plan. Ordinary CLI and API lint use the same `LintTargets` scope regardless of invocation spelling. Ordinary CLI/API lint and LSP call `loader.SelectProjectSources` with frozen targets and ordered candidate lists. Its adapters supply only parsed metadata and Programs; root ranking, extension eligibility, actual source membership and fallback belong to the selector. Config path expansion still validates declared literal paths and glob matches. Selection consumes metadata in declaration order, stopping after each target has its first exact or frozen physical root. Parallel requests prefetch declaration prefixes suggested by the nearest containing config directories, including the first config. This bounds speculative `include` expansion for focused targets. Directory hints affect scheduling only: ordered selection continues beyond a predicted prefix whenever needed. One request-wide limit caps prefetched and on-demand metadata reads together at `GOMAXPROCS`; shared candidates are parsed once, and Program construction reuses that parsed result. Ordered selection can consume each ready result without waiting for all metadata; unused metadata errors are not reported and prefetch never acquires a Program. All metadata workers join before the request closes its Program queue, including on error. Single-threaded requests keep on-demand metadata reads and stop after a usable root. An imported-only candidate does not end the root search, because a later direct root takes priority. JSON, option and extends diagnostics retain lenient parsing behavior.

   Before requesting a Program, the selector checks each target's requested extension against the project's authored compiler options, using the filesystem's case rules. It requests a candidate Program only when at least one pending target qualifies; this does not control the LSP Session's independent project lifecycle. Selected root Programs are acquired concurrently for batch requests; a confirmed eligible root can enter the build queue before the remaining targets' physical identities are resolved. All root selection and prefetch work joins before that queue is closed and its builds are joined. LSP supplies a single-document serial request. If the first explicit metadata root does not admit the target, actual source membership is tried in declaration order before creating a gap; a missing service-selected source remains an error. Different candidate groups can run concurrently, but each group's source search stops once its pending targets are bound. Serial groups stop on the first error. Project selection and final binding share the extension predicate and `programFileIndex`'s exact-source and frozen-identity lookup, seeded with all selected targets. Unknown source identities use bounded per-file VFS realpath queries; directory entries and a parent's realpath cannot establish an overlay file's identity. The selector rechecks authored eligibility per target before accepting a source, so a Program's internal construction options or another target cannot broaden it. The CLI/API binding reuses the chosen owner instead of ranking candidates again. Service construction retains its explicit non-TS root capability. No loader stage changes the target set.

   Program-wide type checking uses the separate `AllDeclared` scope and `config.ResolveTsConfigPathsWithPolicy` to retain the complete raw declaration/default range. In combined mode, that full Program set remains available for checking while every lint target receives its own effective candidate list. This eager path retains its existing root ranking and ordered source binding over already constructed Programs; it does not call the target-driven selector. A pure explicit-project `--type-check-only` run does not scan lint targets. Per-file service/root/reset settings require target discovery to determine their effective values. For type checking, every selected target's root context contributes the owner's complete explicit declaration list, even when its lint binding is disabled or uses service. Service and disabled targets do not trigger an implicit default project. An owner with no selected targets retains the original program-wide declaration/default behavior; there is no file against which to apply scoped overrides.
   ...
   LSP expands explicit project paths from each document snapshot's effective policy; the config catalog does not validate unmatched project declarations. After excluding ignored documents, generation providers distinguish effective project configuration errors from other lint failures. The diagnostics adapter presents those errors on the affected document and clears cached rule diagnostics and fixes; it does not reject configuration updates for unrelated documents. Speculative generation providers use the same error classification without publishing diagnostics. LSP runs the same project selection against its current editor overlay. Its ordinary explicit-project metadata adapter first obtains a Session snapshot; pending document changes, a missing default project, or a dirty project can cause Session Program construction or refresh at that point. A clean snapshot is reused. The shared selector controls which source supplies lint types, not all Session construction, and speculative fixes keep their standalone acquisition path. Editor URI decoding uses tsgo's drive-letter spelling for overlays and file lifecycle events; document identity and protocol responses retain the original URI. Normal diagnostics and speculative fixes call the same project selector as CLI/API: disabled or unmatched targets use a source-only generation and cannot borrow a Session's default module graph. Program acquisition receives the selected parsed config, rather than only its path. Service reuse checks the actual Session Program's construction inputs and referenced config snapshots; a mismatch uses the existing standalone store with the selected metadata. Listed non-TS roots may also require the service construction mode. Ordinary explicit projects use the shared ordered fallback and gap behavior; a missing service-selected source remains an error.

   Existing watcher coverage owns standalone cache invalidation. Acquired candidates remain request-local until the shared selector confirms their source; only then may the store retain a stable Program and record the selected target. Unsuccessful source probes cannot become resident Programs. A cache-admission check also leaves newly built Programs without the requested source transient, before registering source-dependency watchers or rebuilding for watcher coverage. That check still returns the Program to shared selection and cannot choose an owner or declare a gap. Config metadata retains its separate watcher protection. Metadata already observed by a request also constrains resident Program reuse, including reference changes with an unchanged root config. Construction hosts retain only immutable parsed reference snapshots; unknown references still use the compiler host's normal reads and failed-lookup tracking. Metadata observed before watcher coverage stabilized cannot become resident. Speculative fixes keep their private source overlays and do not mutate the editor Session.
...
3. **Lexical + Syntax Parsing**: ts-go tokenizes and parses source files into TypeScript-native AST nodes. Source-only roots additionally run the ts-go binder so syntax-only rules retain symbols and lexical scopes before their rslint Program is published. Source-only construction remains entrypoint-specific: CLI normally parses and binds roots directly, using compatibility compiler Programs for roots unsupported by that parser path; API and LSP assemble source-only generations through compiler Programs. This assembly does not select lint targets or grant type-aware rule eligibility.
...
- automatic Rslint config discovery selects the nearest loadable lint config; explicit config selection uses that config directly. Matching and merging entries determine the target's final project, service, root and reset options. Service discovery starts at the lexical source location; ordinary lint searches the final effective project list using the shared direct-root priority and ordered source lookup described in the pipeline above
- omitting both project settings uses source-only lint without an implicit root `tsconfig.json`. A later matching `project` value replaces the earlier declaration; effective `[]`, `false` or `null` selects no explicit project. Enabled `projectService` may still discover a project with `false` or `null`, but conflicts with an effective project array, including `[]`. Unowned targets use source-only gap linting. Built-in TypeScript presets do not set either project option, matching upstream preset composition. An effective absolute `tsconfigRootDir` anchors explicit project paths and bounds service discovery; without it, explicit paths retain their authored base and the service boundary defaults to the governing config directory (API cwd for inline-only configuration)
...
4. **Lint Target Plan**: Go resolves a stable target set from CLI/API scope, the implicit default baseline, explicit config `files`, global ignores, and `.gitignore`. Ordinary lint and combined `--type-check` prepare targets before loading projects. `--type-check-only` skips the lint scan when no service/root/reset options require effective target configuration
5. **Project Loading**: `program/loader.Session` accepts raw project declarations, frozen targets, and sparse service/root/reset policies projected from the shared file resolver. It assembles one project plan and adds service-selected config metadata before construction. Ordinary CLI/API lint use one target-driven validation and binding policy; program-wide checking retains complete declarations. Explicit and service construction modes remain separate; per-target candidate lists carry scoped root choices and explicit gaps
...
4. **Staged catalog discovery and Program creation**
   ...
   - Ordinary lint consumes each governing config's tsconfig list in declaration
     order. Nearest containing config directories predict a declaration prefix
     to prefetch, including the first config; these hints never decide ownership
     or prevent later on-demand reads. One request-wide limit caps both kinds of
     metadata read together at `GOMAXPROCS`. Each candidate is parsed once, and
     Program construction reuses that result. Errors are reported only
     when selection reaches that candidate. Independent target groups and confirmed
     direct winner Programs may run concurrently. A confirmed eligible root begins construction while
     ...
     serial within each target group so a selected source stops construction of
     later unused projects. Extension eligibility is checked before construction;
     an unsupported target does not become eligible through an imported alias.
...
6. **Program source identity index** (`internal/program/loader`)
   ...
   - Unknown source identities are cached once across the Programs actually
     inspected during that binding pass. Each unknown file uses the VFS realpath
     operation; a parent's realpath and directory entries cannot establish an
     overlay-only file's identity. Per-Program indexes retain
   ...
   - Independent binding realpath lookups use the bounded project-root identity
     worker pool; `--singleThreaded` runs the same work serially.
...
- **LSP Configured-Project Selection Cache**: normal diagnostics and speculative fixes share one root-first selector while keeping separate Program owners. Session projects cache parsed metadata by authored `ParsedCommandLine` generation; the shared selector owns request-local exact/canonical root indexes. Session-external projects retain lightweight parsed metadata only after all config and directory reads have watcher coverage, then reuse that same parse when a Program is needed. Only a Program that contains the requested target becomes resident; non-containing import probes are transient. Config, project-shape, and covered filesystem changes discard affected metadata and Programs. Registration failure and conflicting editor aliases use fresh request-local construction. Speculative fix Programs always remain isolated. This cache lifecycle is independent from CLI/API request sessions; project selection is shared, and no second LSP Session or rslint ParseCache is introduced.
...
- **Program generation**: each published `program.Program` is one logically immutable source, module-resolution, filesystem, and optional-checker generation. Ordinary CLI/API lint and LSP share `loader.SelectProjectSources` for configured-source selection; program-wide type checking retains its eager binding path. CLI/API construct request-local Programs; LSP supplies Session or overlay Programs and retains editor invalidation and cache ownership
