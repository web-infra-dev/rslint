...
- **tsgo distribution**: `pnpm version:crates` updates `tsgo-client`, `Cargo.lock`, `@rslint/tsgo-server`, and all six platform packages to one version. `.github/workflows/release-crates.yml` validates that shared version, assembles the platform packages, and publishes npm and crates.io artifacts together.
