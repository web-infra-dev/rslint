...
2. A directory target's config ancestry is evaluated outer-to-inner. The
   ...
   the node again. Once a parent accepts a child under the current config owner,
   the queued child carries that boundary decision instead of evaluating the
   same global-ignore matcher again. Suspending the child and rebinding it to a
   newly loaded owner invalidates the decision before the frontier resumes.
...
5. **Project Loading**: one request-scoped `program/loader.Session` resolves normalized tsconfig identities once per load. A focused selection execution retains parsed root metadata while choosing direct winners, then builds the deduplicated winner set in parallel; targets with no direct winner advance an ordered fallback frontier. Its output records the complete direct/import/none owner for every target. Full-CWD/no-argument lint and type-check modes retain bounded eager parallel construction. Shared declared paths preserve each active config association and declaration order.
6. **Target Binding**: focused loading projects each target into its already selected Program by exact lexical or canonical filesystem identity and does not repeat project selection; a canonical lookup indexes only that Program. Eager loading derives the same two-tier result—first declared direct root, then first declaration-order import-containing Program—while binding its complete Program set. The loader creates source-only Programs for supported unbound CLI targets, including projects with no tsconfig, and returns one ordered Program sequence plus parallel target projections. CLI/API never coordinate compiler construction or implement ownership themselves
...
6. **Program source identity index** (`internal/program/loader`)
   - Direct lexical Program lookups remain synchronous. The target identity
     maps are not allocated until one misses. Focused binding then indexes only
     the Program already selected for that target. Eager binding builds the
     governing config's Program indexes in one canonicalization batch while it
     derives ownership. Both paths reuse target identities established by
     discovery before resolving unknown source paths; unrelated Programs remain
     untouched.
