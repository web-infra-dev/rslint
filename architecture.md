...
2. **Program Loading**: `internal/config/target` freezes targets, their governing config owners, and path identities without consulting Programs. `config.FileConfigResolver` validates and reuses each target's discovery match when compatible, then merges its final configuration once. `config.ResolveProjectPolicy` projects the effective explicit project declaration and service/root/reset options from that same immutable result; it performs no matching, filesystem lookup, or project construction. CLI/API use `config/lint.Resolver` both before construction and after source binding, sharing its file and matched-shape caches. Before construction, file configuration resolves in parallel with at most `GOMAXPROCS` workers; policy projection and failures retain target input order. `--singleThreaded` keeps this preparation serial. Workers finish before the request proceeds, including when a file fails. LSP consumes its existing document snapshot's resolved config. Node only evaluates and serializes values; presets remain ordinary reusable values.
   ...
   `program/loader.Session` assembles one project plan. Broad plain lint and focused CLI/API requests share construction slots and use target membership to select direct owners. Broad lint still validates every effective candidate and ranks direct roots by exact and physical identity before construction; focused requests retain their existing incremental root search and validation range. Targets absent from every applicable parsed root set become source-only gaps without constructing import candidates. The shared CLI/API binder enforces this root requirement even when other targets or program-wide checking retained a Program that imports the file. A listed root rejected by the compiler remains a separate compatibility boundary: broad ordinary lint retains parallel ordered source fallback for that target, while focused selection reports its existing missing-source error. Broad and AllDeclared binding retain the existing fallback only after a root matched; unmatched targets never enter it. Program-wide type checking uses `config.ResolveTsConfigPathsWithPolicy` to retain the complete raw declaration/default range. In combined mode, that full Program set remains available for checking while every lint target receives its own effective candidate list. A pure explicit-project `--type-check-only` run does not scan lint targets. Per-file service/root/reset settings require target discovery to determine their effective values. For type checking, every selected target's root context contributes the owner's complete explicit declaration list, even when its lint binding is disabled or uses service. Service and disabled targets do not trigger an implicit default project. An owner with no selected targets retains the original program-wide declaration/default behavior; there is no file against which to apply scoped overrides.

   Shared predicates do not choose project order or construction scope.
   `program.RootFileIndex` supplies focused explicit-project root membership
   to both loader and LSP. Extension admission remains in the selected project
   construction and does not substitute for root membership. Each selector owns the ordered
   ...
   The loader appends selected config metadata to its existing project plan before construction. Config parsing uses a metadata-only host; source snapshots are bound only when Program construction has chosen its serial or parallel filesystem view. Ordinary and service modes share scheduling, source snapshots and one target-binding pass. Target-driven construction consumes service discovery's selected owner directly, then validates its exact source; it does not independently reselect service ownership using ordinary batch matching. The AllDeclared construction path validates service sources before publishing its complete type-check set, including when lint binding will be skipped. Complete service Programs use source references and remain distinct from explicit Programs with different construction options. Every lint target has an explicit candidate list, including an empty list for gaps. Constructed projects retained for other targets or type checking cannot supply types outside that list. A sole explicit candidate still requires parsed root membership; import membership cannot select a lint project. Service candidates never leak into another target's list.

   LSP expands explicit project paths from each document snapshot's effective policy; the config catalog does not validate unmatched project declarations. After excluding ignored documents, generation providers distinguish effective project configuration errors from other lint failures. The diagnostics adapter presents those errors on the affected document and clears cached rule diagnostics and fixes; it does not reject configuration updates for unrelated documents. Speculative generation providers use the same error classification without publishing diagnostics. LSP runs the same metadata discovery against its current editor overlay before requesting a Program. Explicit selection stops after the ordered root scan; it never acquires configured Programs to probe import-only membership. Editor URI decoding uses tsgo's drive-letter spelling for overlays and file lifecycle events; document identity and protocol responses retain the original URI. Normal diagnostics and speculative fixes share the complete binding policy: disabled or unmatched targets use a source-only generation and cannot borrow a Session's default module graph. Program acquisition receives the selected parsed config, rather than only its path. Service reuse checks the actual Session Program's construction inputs and referenced config snapshots; a mismatch uses the existing standalone store with the selected metadata. Listed non-TS roots may also require the service construction mode. A missing source in an otherwise compatible selected Program remains an error.
...
3. **Lexical + Syntax Parsing**: ts-go tokenizes and parses source files into TypeScript-native AST nodes. Source-only roots additionally run the ts-go binder so syntax-only rules retain symbols and lexical scopes before their rslint Program is published.
   ...
   Construction and capability boundaries remain separate. CLI preparation returns supported gap roots as ordered `program.SourceSet` inputs, retaining the loader's grouping and request-local source snapshots without parsing their ASTs. A SourceSet owns immutable construction inputs, not a partial Program: it can build its complete source universe or one independent file. The linter decides which lifetime its rules and requested artifacts permit. Unsupported roots keep compiler compatibility admission. API retains that compiler construction for all gap roots and adapts its bound sources with `program.NewFromBoundSources`, which exposes no checker capability. Both compiler paths already reuse `utils.CreateProgramFromOptionsLenient` for parsing and binding. LSP uses that same helper for one fallback document over its editor or speculative overlay, keeps the compiler-backed facade, and filters type-aware rules when the document lacks a selected project. It does not borrow the CLI/API loader's snapshots or make gap Programs resident. The shared options do not select projects, create hosts, schedule work, cache across generations, or determine facade capabilities.

   For ordinary native observations without retained file or edit artifacts, the core freezes deferred files' configuration by path, then bounded workers parse, bind and lint one file at a time. The same rule traversal and listener registry serve retained and deferred Programs. A worker detaches its diagnostics from ASTs before publishing them and taking another file; input queues retain paths and configuration only. No fixed-size batch or runtime GC setting controls this lifetime. When any eligible rule requires the complete Program, the entire original root set is materialized, including zero-rule roots. Type checking, plugin work, requested AST/edit artifacts and in-memory fix snapshots retain complete materialization. Native projects keep their existing source and checker scope.
...
`RequiresProgram` declares a dependency on other roots of the source universe,
even without a checker. The configured-rule descriptor preserves it; a deferred
source set with any eligible such consumer is built in full before planning.
The import catalog conservatively requires this scope, because its resolution
fallback as well as export and cycle analysis can consult other loaded files.
Rules without that requirement may receive a one-file source-only Program;
filesystem and package lookup remain available through that same authority.
...
5. the linter appends them, in rule order, to the executing worker's sparse
   dispatch registry
...
Before publishing an observation or releasing its generation, the pipeline
replaces diagnostic AST references with shared text-only projections. This
happens after fix-source identity validation and generation text freezing;
native and joined plugin diagnostics for the same exact source share one
projection, while different source objects are never merged by path. Initial
and final autofix observations therefore retain their own text without keeping
obsolete ASTs alive. Explicitly requested `LintedFile` projections still retain
their source generations for consumers that require them.
...
- automatic Rslint config discovery selects the nearest loadable lint config; explicit config selection uses that config directly. Matching entries determine the effective explicit project, service, root and reset options. Service discovery starts at the lexical source location; ordinary selection uses the first matching parsed root in the effective project list. Import-only membership cannot select a lint project
- omitting both project settings leaves ordinary lint source-only; the historical config-directory default applies only to program-wide type checking. Effective `project: false`, `null`, or `[]` clears the target's ordinary explicit binding; `projectService: true` instead discovers configured ownership; unowned files use source-only gap linting. Built-in TypeScript presets do not set either project option, matching upstream preset composition. `tsconfigRootDir` is an absolute ancestor-search boundary, not a forced project, and defaults to the governing config directory (API cwd for inline-only configuration)
...
- within each Program-registry build, normalized declared tsconfig paths are deduplicated across config associations; each CLI autofix observation creates a new Program generation over the core-owned memory overlay and repeats the original construction scope without writing the base filesystem. Parsed-root lint ownership is therefore recomputed for each source generation; changing imports alone cannot promote an unmatched lint target to a typed project. File-symlink declarations remain distinct because TypeScript resolves relative paths from the declared location. Selected CLI files outside the governing config's parsed tsconfig root sets are parsed and bound as independent rslint Programs. Targets whose names collide under a case-insensitive ts-go path key are partitioned across independent Programs so distinct physical files and package scopes remain distinct
...
6. **Target Binding**: the loader locates the selected project's AST by lexical or frozen canonical identity without changing project policy or target scope. Disabled and unmatched targets, including explicit import-only files and service misses, receive source-only inputs. They cannot borrow a Program selected by another target's parser policy. Integrations receive retained Programs with exact parallel target projections and, for CLI preparation, deferred root sets with the same frozen source/target mappings. They never construct compiler hosts or implement ownership
...
8. **Rule Plan Preparation**: for each acquired generation, the core calls `PrepareLintPlan()` with one ordered Program sequence, its exact parallel target projection, and the generation's rule resolver. Planning never discovers, excludes, or scans for fallback targets; a target absent from its bound Program is an invariant error. The immutable result owns that Program sequence, freezes rare syntax diagnostics in a sparse plan-level projection, and keeps each hot per-file execution unit limited to its source, resolved rules, shared rule environment, and checker capability. Syntax-error and zero-rule files remain selected, and the same non-empty file/rule projection feeds third-party plugin dispatch. Worker results are joined in stable Program/file order; cancellation cannot publish a partial plan, and callback panics return through the caller so the enclosing generation lifecycle remains exact-once.
   Before this retained-Program planning, the core classifies deferred root sets
   from their path-based configuration and observation demands. Independent
   file descriptors join the same plan without ASTs; other sets are materialized
   in full and follow the retained path. Deferred syntax checks happen when a
   worker builds the file, still suppressing its rules and preserving file counts.
...
4. **Staged catalog discovery and Program creation**
   ...
   - Discovery commits only the config catalog in stable order. Lint then
     ...
     construction selects parsed roots and leaves unmatched targets as gaps.
   ...
   - Focused lint parses each governing config's tsconfig frontier in declaration
     ...
     may run concurrently; ownership results remain in declaration order so a
     later completion cannot overtake an earlier project. Unmatched files do
     not schedule import probes.
...
- **Task-Local Listener Reuse**: each checker-shard task or deferred-file worker owns one sparse listener registry and reuses its map and per-kind slice capacity across that task's serial files. Registries are never shared across tasks or requests
...
- **Resolver-Scoped Effective Config Plans**: each `FileConfigResolver` owns
  ...
  complete initialization. They are discarded with the resolver. CLI/API
  autofix generations within one request reuse immutable config results while
  replacing source mappings; separate requests get new resolvers. LSP reuses
  resolvers owned by its committed config catalog until a config refresh
  replaces them. Program and filesystem snapshots retain their separate
  generation lifetimes. Merged config maps and configured-rule slices returned
  by the resolver are immutable shared state.
...
- **Bounded Listener Retention**: a listener registry lives only for one checker-shard task or deferred-file worker. After each file it clears every function slot before shortening the slices, so backing capacity can be reused without retaining closures, source files, checker state, or rule contexts. The registry is dropped when that task completes and is never pooled across runs or LSP requests
- **Demand-Driven Source Lifetimes**: a deferred source worker owns one complete parse/bind/lint scope. SourceSets share source text, not a cache of completed bound ASTs. Each worker publishes text-backed diagnostics before taking its next descriptor. The worker count is bounded by available parallelism, and single-threaded execution uses one worker. Cancellation stops new admissions; cancellation, errors, panics and abnormal worker exits join every worker before diagnostic channels or the generation lease can close. No forced collection or cross-run pool is involved.
...
- **Program generation**: each published `program.Program` is one logically immutable source, module-resolution, filesystem, and optional-checker generation. CLI/API prepare construction through `internal/program/loader`; deferred `program.SourceSet` inputs retain immutable roots and a source host, while the core chooses complete or independent materialization from consumer requirements. LSP adapts its session or isolated overlay without exposing the private backend
- **Lint plan**: retained files bind exact AST generations and freeze rules, shared environments and checker eligibility. Deferred entries freeze root identities and path-based configuration without ASTs; only their materialization and syntax checks wait for execution. Execution never discovers targets or resolves configuration again
...
- **Program Loader Session**: Request-scoped `internal/program/loader` owner that builds configured projects, binds target-plan files, and returns complete Programs or deferred source-only construction inputs with their lint projections to CLI/API
