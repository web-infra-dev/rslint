...
│  │  │  cmd/rslint program assembly         │  │  internal/lsp            │  │  │
...
| `cmd/rslint/`                  | Main Go binary entry point with CLI, API, and LSP modes                                                                                     | JS/TS config CLI path is `internal/config/discovery -> internal/config -> lint-target/program assembly -> internal/linter`; `program_build.go`, `lint_target_*.go`, and `gap_programs.go` keep those assembly responsibilities separate. JSON config starts at `internal/config`. `--api` is consumed by `packages/rslint` and `packages/rslint-wasm`; `--lsp` is consumed by `packages/vscode-extension` |
...
| `internal/utils/`              | Shared utilities for JSONC, compiler hosts, invocation-scoped compiler construction, overlay VFS, and AST/type helpers                      | `ProgramBuildContext` owns the final CLI/API VFS view plus source/AST and metadata/config caches used by command-side Program assembly; LSP, rule tests, and auxiliary entry points reuse the lower-level compiler helpers. Source-generation module resolution belongs to `internal/program`                                                                                                             |
...
├───────────────────────────────────────────────────────────┤
│ Configuration / Program Assembly / IPC / Inspector        │  ← internal/config/, cmd/rslint/*program*.go,
