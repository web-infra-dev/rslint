...
2. **Program Loading**: `internal/config/target` freezes the exact targets and config path spaces. `internal/config.ProjectPolicyResolver` merges only each target's parser policy; it does not resolve rules. Boolean `projectService` delegates configured-project ownership to `internal/program/projectservice`, shared by CLI/API and LSP. The selector owns lexical ancestor discovery, reference order, and membership; TypeScript still parses configs and constructs complete Programs. It uses compiler source-reference identity, checks sibling references before their subtrees, and keeps one caller-owned generation of candidate metadata and Programs. A service target without a configured project is an error. Explicit project paths conflict with enabled service after flat-config merging, including empty arrays. Disabled and explicit targets cannot borrow service Programs.
   ...
   Service-mode type checking discovers projects from the selected scope and checks their complete Programs. Legacy explicit configs keep program-wide construction, including inactive owners. The config resolver uses prepared matcher scopes to recognize an unconditional service disable overriding earlier preset settings; the loader does not interpret config matching. Mixed automatic policies and authored root boundaries require real targets to resolve. Source-only fallback remains available for disabled or legacy unbound targets. Configured service Programs preserve the initial parsed-config identity when a reference cycle returns to that config, so compiler source redirects do not suppress its diagnostics.
...
6. **Target Binding**: the loader locates the selected project's AST by lexical or frozen canonical identity without changing project policy or target scope. Disabled and legacy unbound targets receive source-only Programs. Unowned service targets fail. CLI/API receive one Program sequence and exact parallel target projections; they never construct compiler hosts or implement ownership
