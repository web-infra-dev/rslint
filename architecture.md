...
| `internal/api/server/`         | Concrete rslint API request handlers                                                                                   | Implements lint and AST-inspection requests over `internal/api`, owns API-specific paths, mutable fix overlays, per-generation Program/plugin orchestration, structured responses, reverse config/plugin adapters, and the process-wide AST inspection cache; `cmd/rslint` composes its `Handler` with the parent package's generic `Service`                                                                              |
...
while retaining its stale-generation and code-action lifecycle. Surface counts,
fix materialization and storage, Program/plugin orchestration, path bases, stderr
notices, and protocol empty-array rules therefore do not belong to a universal
diagnostic pipeline. Only the transport-neutral bounded pass lifecycle is shared.
...
- **LSP normal diagnostics**: request all native edits for editor surfaces
- **LSP speculative fix-all passes**: request native autofixes only
- **API diagnostics**: materialize all native and third-party plugin fixes and
  suggestions as response metadata, independently of whether `fix: true` applies
  autofixes
- **API**: `lint({ fix: true })` applies fixes to a request-local mutable overlay,
  rebuilds Programs and native/plugin plans until stable or ten writable passes,
  and returns the final source per changed file in `output` (the JS side persists
  it via `Rslint.outputFixes`). Diagnostics, counts, and encoded sources come from
  the final generation; the API never writes the overlay to disk.

`internal/linter.RunAutofixLoop` owns only the shared convergence boundary: ten
writable passes, stable-stop behavior, error propagation, and an optional final
read-only verification after the tenth write. CLI and API enable that verification
because they return final diagnostics; LSP fix-all returns content only and skips
it. Each integration continues to own Program reconstruction, native/plugin
dispatch, and persistence to disk, an in-memory overlay, or an editor edit.
...
10. **Fix Passes**: CLI `--fix` writes each changed generation to disk, while API `fix: true` updates every known target/canonical/source alias in its request-local overlay. Both use the shared bounded lifecycle, rebuild Programs and prepared native/plugin plans, and rebind the unchanged target plan after each pass. A file may be owned by a different Program generation after its import graph changes; Program objects are immutable identities and are never refreshed in place. LSP fix-all uses the same bound but rebuilds isolated single-document overlays and returns only final content
...
6. **Program source identity index** (`internal/program/loader`)
   ...
   - Unknown source identities are cached once across the Programs actually
     ...
     only canonical identities present in the target plan. CLI and API fix passes
...
- **Server-Owned Debounced Re-linting**: `refreshCh` and `debounceCh` collapse bursts of file changes and session refreshes onto the main dispatch loop. The server remains the single owner of the 200 ms typing debounce; open and save diagnostics stay immediate, while save and close discard redundant pending work for their target document. A speculative fix-all request preserves pending diagnostics because it does not publish or update the diagnostic cache itself.
...
- **Generation-Based Fix Invalidation**: after every CLI fix write attempt, the compiler host atomically installs an empty source generation before any Program rebuild. API fix passes batch-update all known lexical, canonical, and Program-source aliases in the request overlay, then perform the same generation swap before rebuilding. Swapping generations rather than clearing a live map prevents an older in-flight read from repopulating the new generation. LSP speculative fixes instead rebuild isolated document overlays and leave the session document version unchanged.
...
- **Bounded Multi-Pass Fixing**: CLI `--fix`, API `fix: true`, and LSP `fixAll` use `internal/linter.RunAutofixLoop` to cap the cascade at ten writable passes. CLI and API add a read-only verification only when the tenth pass still changed source; LSP returns content and does not need that extra lint.
