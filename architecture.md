...
its zero value means the moving `latest` edition. It separately validates and
extracts the authored top-level `languageOptions.sourceType` (`module`,
`script`, or `commonjs`) and forwards it unchanged to each native
`RuleContext`; an absent value remains empty so a rule can fall back to
structural module detection. The legacy `parserOptions.sourceType` location is
not read. `sourceType` is rule metadata and does not change TypeScript parsing
or compiler module resolution. The linter uses the normalized edition to build
...
contribute no defaults. The resolver does not inspect `package.json` or the
authored `sourceType`: the latter is forwarded directly to rules rather than
changing language defaults. A rule must explicitly consult
`RuleContext.SourceType` to honor it; rules that use structural module
detection retain that behavior until migrated.
