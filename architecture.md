...
- selected files outside tsconfig coverage do not enable type-aware rules and
  are skipped by the type-check phase; CLI targets with no executable rules use
  the equivalent syntax-only parser path, while all other callers use fallback
  Programs
...
- within each Program-registry build, normalized declared tsconfig paths are deduplicated across config associations; CLI fix passes create a new registry build. File-symlink declarations remain distinct because TypeScript resolves relative paths from the declared location. Selected files outside the governing config's Programs receive a non-project-backed fallback Program whenever any gap has an executable rule. If every CLI gap is ruleless after the type-information gate, the same collision groups are parsed directly for syntax diagnostics and exact linted-file accounting instead
...
6. **Program Binding**: each target is bound by exact lexical or canonical filesystem identity to the first containing Program declared by its governing config. The CLI resolves rules for unbound targets before choosing their representation: one executable gap rule creates the unchanged non-project-backed fallback set, while an entirely ruleless gap set uses transient syntax-only SourceFiles. API unbound targets remain fallback-Program-backed
...
- **Gap-File Degradation**: gap files skip type-aware rules and semantic diagnostics instead of paying unreliable semantic costs. When every CLI gap is ruleless, bounded workers parse only the selected roots, reproduce the fallback Program's syntactic diagnostics, and let valid transient ASTs become collectible immediately; any executable gap rule keeps the existing all-gap fallback Program semantics
