...
4. **Staged catalog discovery and Program creation**
   ...
   - Ordinary lint consumes each governing config's tsconfig list in declaration
     order. One request-wide pool prefetches metadata for the distinct candidates
     referenced by target groups, with at most `GOMAXPROCS` concurrent reads.
     Ordered consumers wait for each result instead of starting additional reads;
     Program construction reuses the same parsed config. Errors are reported only
