...
| `internal/program/projectservice/` | Configured TypeScript project discovery shared by CLI/API and LSP                                                              | Owns candidate order and membership only. Accepts a filesystem generation and parse/build callbacks; does not own config matching, lint targets, rule resolution, editor lifecycle, or compiler parsing                                                                                                                                                                                                                                     |
...
2. **Program Loading**: `internal/config/target` freezes the exact targets and config path spaces. `internal/config.ProjectPolicyResolver` merges only each target's parser policy; it does not resolve rules. The Node config boundary preserves TS-preset origin metadata separately from authored parser options. Each resolver collects its composed config's candidates once; after matching and merging it resolves an explicit `tsconfigRootDir`, an unambiguous inferred directory, or the caller's cwd, in that order. CLI/API pass cwd into Program loading; LSP freezes it in the document snapshot. Preset getters carry provenance on returned entries and rules, so ordinary spreads and module caching retain it without a process-global candidate registry. Boolean `projectService` delegates configured-project ownership to `internal/program/projectservice`, shared by CLI/API and LSP. The selector owns lexical ancestor discovery, reference order, and membership; TypeScript still parses configs and constructs complete Programs. It uses compiler source-reference identity, checks sibling references before their subtrees, and keeps one caller-owned generation of candidate metadata and Programs. A service target without a configured project remains unbound and uses the integration's existing source-only gap path. Discovery returns an empty selection for ordinary misses and errors for actual config or Program failures; it does not decide fallback capabilities or execute rules. Explicit project paths conflict with enabled service after flat-config merging, including empty arrays. Disabled and explicit targets cannot borrow service Programs.

   Legacy explicit-project mode retains declaration-order direct-root selection before import fallback. Focused lint materializes only needed projects; broad plain lint builds active owners' declarations. `program/loader.Session` owns CLI/API source, metadata, parse, and extended-config caches and returns the exact target binding plus Program facades. LSP supplies its own versioned editor overlays and existing resident Program store, distinguishing explicit and source-reference Program modes. Project-shape events invalidate resident metadata and Programs; source changes use `Program.UpdateProgram` where watcher coverage permits. Speculative fixes remain isolated. Neither integration imports the other's lifecycle or duplicates the shared service selection policy.

   Service-mode type checking discovers projects from the selected scope and checks their complete Programs. Legacy explicit configs keep program-wide construction, including inactive owners. The config resolver uses prepared matcher scopes to recognize an unconditional service disable overriding earlier matching entries; the loader does not interpret config matching. Mixed automatic policies and authored root boundaries require real targets to resolve. Source-only fallback handles disabled targets and unbound targets from either project policy. It does not narrow another selected Program's complete type-check scope. Configured service Programs preserve the initial parsed-config identity when a reference cycle returns to that config, so compiler source redirects do not suppress its diagnostics.

   Configured service construction copies the parsed command line and its compiler options before applying TypeScript's `allowNonTsExtensions` setting. This happens after root discovery and preserves `allowJs`, the complete roots, and the config source identity. Listed and triple-slash referenced JS files can then belong to a service Program without changing legacy Programs, config glob expansion, or ordinary import filtering. Service ownership uses actual Program membership instead of rejecting these files from their original compiler options.
...
- automatic Rslint config discovery selects the nearest loadable lint config; explicit config selection uses that config directly. Its effective parser policy then selects either automatic TypeScript project discovery or legacy explicit projects. Automatic discovery starts at the lexical source location; legacy projects prefer the first declared direct root, then the first import-containing Program
- omitting both project settings retains the config-directory `tsconfig.json` fallback. Explicit `project: false`, `null`, or `[]` disables it; `projectService: true` instead discovers configured ownership; unowned files use source-only gap linting. Built-in TypeScript presets do not set either project option, matching upstream preset composition. `tsconfigRootDir` is an absolute ancestor-search boundary, not a forced project, and defaults to a TS-preset config origin or the invocation cwd
...
- `--type-check` and `--type-check-only` check complete configured Programs. Legacy explicit configurations build all declarations, including owners with no lint targets. Owners with automatic discovery, mixed policies, cleared projects, or `tsconfigRootDir` first resolve their selected targets to projects. Only configured Programs expose program diagnostics; type-check-only never resolves or executes lint rules
...
5. **Project Loading**: `program/loader.Session` applies the config-owned per-target parser policy. Service targets use the shared configured-project selector and source-reference-enabled complete Programs. Legacy targets retain focused or broad declaration loading according to CLI scope. The loader shares invocation caches and freezes final ownership, including intentionally unbound targets
