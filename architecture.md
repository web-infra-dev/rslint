...
5. **Project Provider**: one request-scoped `program/loader.Session` interns exact lexical tsconfig IDs and owns separate parse-metadata and Program-construction stages. Directory coverage may ask it to prefetch candidates; a file/scoped request otherwise stays demand-driven, except for at most one metadata-proven direct-project scheduling hint. `--type-check` adds the per-owner catalog as a separate consumer role. These choices affect scheduling and materialization only.
...
4. **Staged catalog discovery and Program creation**
   ...
   - The request provider interns exact lexical project identities in stable
     ...
     projects, and optional directory-prefetched candidates use one bounded
     ...
     target direct; distinct per-target hints disable speculation. Parse/build
