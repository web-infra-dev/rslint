...
| `internal/config/`             | Configuration models, JSON loading, matching/merging, runtime ownership resolution, lint-target planning, and configured-rule evaluation | Owns the shared authored-global-ignore matcher consumed by both discovery phases. File resolvers receive an immutable `internal/rule.Catalog` explicitly and convert merged rule settings into execution descriptors without owning concrete rule registration                                                                                                                      |
...
| `internal/rule/`               | Rule framework, immutable rule catalogs, configured-rule descriptors, context, diagnostics, fixes, and disable manager                   | Shared foundation for config resolution, core rules, and plugin rules; `internal/linter` consumes its immutable rule environments, listeners, reporting APIs, and Program-derived cache helper                                                                                                                                                                                      |
...
| `internal/rules/`              | Core lint rule implementations without plugin namespace; `all.go` aggregates them into the `GetAllRules()` slice                         | `internal/rules/catalog` combines this slice with native plugin rules once, returning an immutable catalog executed by `internal/linter`                                                                                                                                                                                                                                            |
| `internal/rules/catalog/`      | Native rule-catalog assembly and ESLint-plugin placeholder overlays                                                                      | Imports concrete Go rule collections, caches the immutable native catalog, and derives isolated CLI run, API request, or LSP config-generation catalogs without mutating shared state                                                                                                                                                                                               |
| `internal/plugins/typescript/` | `@typescript-eslint`-style rules                                                                                                         | Contributes rules to `internal/rules/catalog`, often relying on `TypeChecker` from `typescript-go`                                                                                                                                                                                                                                                                                  |
| `internal/plugins/react/`      | React rule implementations                                                                                                               | Contributes rules to `internal/rules/catalog` and executes through the same listener pipeline in `internal/linter`                                                                                                                                                                                                                                                                  |
| `internal/plugins/jest/`       | Jest rule implementations                                                                                                                | Contributes rules to `internal/rules/catalog` and executes through the same listener pipeline in `internal/linter`                                                                                                                                                                                                                                                                  |
| `internal/plugins/import/`     | Import plugin registration and rules                                                                                                     | Contributes rules to `internal/rules/catalog` and participates in normal config-driven linting                                                                                                                                                                                                                                                                                      |
...
1. `internal/rules/catalog` supplies an immutable catalog for the CLI run, API request, or committed LSP config generation
...
**Key difference**: JSON configs are normalized against the explicitly supplied native rule catalog, which auto-enables core rules and rules from declared native plugins unless explicitly overridden. JS/TS configs only enable what the normalized config entries specify, usually via presets.
...
- native and third-party plugin rules are gated by their normalized prefixes for JS/TS configs; each CLI run and API request derives its own native-plus-plugin catalog, while LSP commits one such catalog with the matching Node generation. Replacing or removing plugins therefore replaces the catalog instead of retaining process-wide placeholders
...
│                                                           │     internal/rules/catalog/, internal/plugins/
├───────────────────────────────────────────────────────────┤
...
- **Config → Program Loader**: config owns target discovery, config ownership, and the stable lint-target plan; the loader owns configured project construction and target-to-Program binding
...
- **Rule Catalog**: Immutable name-indexed snapshot of rule implementations. `internal/rules/catalog` assembles the shared native snapshot and derives isolated ESLint-plugin overlays; config resolvers receive the appropriate snapshot explicitly
