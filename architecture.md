...
2. **Program Loading**: `internal/config/target` freezes targets, their governing config owners, and path identities without consulting Programs. `config.FileConfigResolver` matches and merges each file's configuration once. `config.ResolveProjectPolicy` projects service/root/reset options from that same immutable result; it performs no matching, filesystem lookup, or project construction. CLI/API use `config/lint.Resolver` both before construction and after source binding, sharing its file and matched-shape caches. LSP consumes its existing document snapshot's resolved config. Node only evaluates and serializes values; presets remain ordinary reusable values.
   ...
   Ordinary `project` strings and arrays keep the governing owner's ordered declarations. `config.ResolveTsConfigPathsWithPolicy` expands authored patterns against each entry's literal base; an effective absolute `tsconfigRootDir` overrides that base. Omission or a null reset retains the original bases. Service discovery instead defaults its search boundary to the already-selected config owner, or the API's existing cwd for inline-only config. Imports, preset composition and `basePath` do not redefine that owner.
   ...
   `internal/program/projectservice` adds configured-project selections only for targets with effective `projectService: true`. It owns lexical ancestor discovery, reference order, and membership; the caller supplies TypeScript config parsing and complete Program construction. A normal miss returns no selection and follows the existing source-only gap path; actual config and construction failures remain errors. Effective explicit project settings, including empty arrays, conflict with enabled service. The selector shares no lifecycle with an integration: CLI/API supply request-local hosts, while LSP supplies versioned editor overlays and its resident Program store.

   The loader merges ordinary and service results into one `ProjectSet`, then binds once. Explicit paths are deduplicated within their construction mode. Service Programs use source references and remain separate from explicit Programs of the same tsconfig. Sparse per-target candidates preserve scoped root and disabled/service selections; absent overrides use the original owner declaration order. Service Programs never enter ordinary owner candidates. Config, Program construction, source binding, gap capability, and rule execution remain separate responsibilities. LSP keeps its existing watcher-driven metadata and Program lifecycle, with speculative fixes isolated from resident generations.

   Configured service construction copies the parsed command line and its compiler options before applying TypeScript's `allowNonTsExtensions` setting. This happens after root discovery and preserves `allowJs`, the complete roots, and the config source identity. Listed and triple-slash referenced JS files can then belong to a service Program without changing explicit Programs, config glob expansion, or ordinary import filtering. Service ownership uses actual Program membership instead of rejecting these files from their original compiler options.
