...
| `internal/program/projectselection/` | Entry-neutral configured-project ownership policy                                                                                           | Selects one complete `direct` / `import` / `none` binding for each config-owned target from ordered project metadata and Program-membership evidence. CLI/API loaders and LSP provide different lifecycle adapters, but do not implement another ownership state machine                                                                                       |
| `internal/program/loader/`           | Run-scoped Program construction and lint-target binding for CLI/API                                                                         | Consumes config-owned lint-target plans, supplies focused or eager project evidence to `internal/program/projectselection`, and projects its complete result into one backend-agnostic Program sequence. Construction choices and ts-go compatibility details remain private; LSP keeps its resident Programs and uses a separate adapter to the same selector |
...
```text
CLI/API focused ── request-scoped lazy provider ─┐
CLI eager       ── prebuilt Program provider ───┼─→ projectselection ─→ complete owner binding
LSP             ── resident Session provider ───┘

Target discovery, Program lifetime, fix/type-check loops, and error transport
stay in their entry adapters; only configured-project ownership is shared.
```
...
2. **Program Loading**: `internal/config` first resolves a stable, config-owned lint-target plan and the ordered tsconfig declarations. `internal/program/projectselection` is then the single ownership policy for CLI, API, and LSP: every target first selects the earliest declared direct root, otherwise the earliest declaration-order Program that imports it and supports its extension, otherwise `none`. Exact lexical paths and authoritative physical identities are both evidence; construction or completion order never changes the winner. The selector returns the complete binding, so later loading projects that result into lint inputs never performs a second ownership scan. Focused CLI/API adapters consume metadata evidence in declaration order, may prefetch it without publishing a decision, reuse it for selected Program construction, and preserve the reached-error frontier. Eager CLI adapters construct the configured catalog first and present those already-built Programs to the same selector. Program-wide type-check modes still build every project declared by the effective loaded config catalog. CLI/API targets with `none` become source-only rslint Programs with parser, binder, package/module metadata, and direct-import resolution but without recursively loading a project graph. A request-local implementation of ts-go's `ExtendedConfigCache` reuses common `extends` parses without modifying ts-go, while root and project-reference configs are explicitly registered for raw-read snapshots. LSP keeps its resident ts-go Session and uses an LSP adapter to the same selector for normal diagnostics and speculative fixes; it does not create a request-scoped loader Session. An LSP `none` result continues through its existing inferred/default or isolated fallback Program and remains non-type-aware for rule eligibility. Session-owned projects expose their authored command-line roots through a generation cache; unloaded custom projects expose watcher-protected parsed metadata, and Program construction reuses that exact parse. Lexical tsconfig declaration paths remain distinct so symlink-relative includes keep TypeScript's declared-path meaning. For a declared custom tsconfig that Session has not loaded, rslint can retain the existing session-external ts-go Program by exact config path and advance source-only edits through ts-go's `Program.UpdateProgram`; fallback Programs that do not contain the target remain transient. Project-shape events discard resident metadata and Programs, and dependency reads are registered through ts-go's watcher mapper before either becomes resident. If watcher coverage cannot be established, rslint transparently keeps fresh request-local construction. Speculative fix Programs remain isolated and request-local.
...
5. **Project Loading**: one request-scoped `program/loader.Session` resolves normalized tsconfig identities once per load. Focused loading supplies lazy metadata and Program providers to the shared project selector. A recursive CLI scope supplies prebuilt Programs only when its target directories cover every active config owner; no-argument scope is first normalized to the invocation CWD. File-only scope, an uncovered parent owner, or an automatic-catalog owner activated only by an explicit file remains focused. Program-wide type-check remains eager and independent from lint-target ownership. Shared declared paths preserve each active config association and declaration order.
6. **Target Binding**: `internal/program/projectselection` returns a complete direct/import/none owner vector. The CLI/API loader consumes that vector once, creates source-only Programs for supported `none` targets, and returns one ordered Program sequence plus parallel target projections. CLI/API entry points never inspect Program membership, and `LoadCLI`/`LoadAPI` never re-select a target from the retained Programs
...
4. **Staged catalog discovery and Program creation**
   ...
   - Focused lint consumes each governing config's tsconfig metadata in
     declaration order, while the provider may prefetch evidence without
     publishing a decision. Independent config frontiers and confirmed direct
     winner Programs may run concurrently; import-only fallback remains serial
     within one governing config so a later completion cannot overtake an
     earlier project.
...
- **LSP Configured-Project Selection Cache**: normal diagnostics and speculative fixes use the shared entry-neutral project selector while keeping separate Program owners. The LSP adapter supplies session-resident metadata and Programs instead of the CLI/API request loader. Session projects cache exact/canonical root membership by authored `ParsedCommandLine` generation. Session-external projects retain lightweight parsed metadata only after all config and directory reads have watcher coverage, then reuse that same parse when a Program is needed. Only a Program selected for the requested target becomes resident; non-containing import probes are transient. Config, project-shape, and covered filesystem changes discard affected metadata and Programs. Registration failure and conflicting editor aliases use fresh request-local construction. Speculative fix Programs always remain isolated; no second LSP Session or rslint ParseCache is introduced.
...
- **Config → Project Selection**: config owns target discovery, config ownership, and the stable lint-target plan; `internal/program/projectselection` owns ordered project ownership and emits a complete direct/import/none binding
- **Project Providers → Project Selection**: CLI/API request loaders and the resident LSP session own metadata, Program construction, caching, invalidation, and concurrency. They supply evidence to the selector without changing its winner or error-frontier policy
- **Program Loader → Integrations**: CLI/API receive one ordered rslint Program sequence plus parallel target/path projections and never select or inspect a Program backend; LSP wraps its session-owned Programs directly after using the same selector
...
- **Program Loader Session**: Request-scoped `internal/program/loader` owner that builds configured projects, binds config-owned lint targets, creates source-only generations when needed, and returns one unified Program sequence to CLI/API
