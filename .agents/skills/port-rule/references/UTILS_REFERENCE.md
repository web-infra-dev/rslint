...
- To check locality (e.g. "is this callee shadowed by a local declaration") once `Resolve` returns a symbol, pair it with `utils.IsSymbolDeclaredInFile(symbol, ctx.SourceFile)` (`internal/utils/ts_eslint.go`): it walks `symbol.Declarations` for one in this file.
- For built-in **value** shadowing specifically, use `utils.IsValueSymbolDeclaredInFile` instead: it skips interfaces, type aliases, and non-instantiated namespaces — declarations that merge into an ambient global's symbol without shadowing its value (`interface Map {}` in a global script attaches to the same symbol as lib.d.ts's `Map`, but doesn't make a call to `Map()` local).
