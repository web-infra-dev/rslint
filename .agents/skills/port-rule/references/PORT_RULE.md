...
Note: ESLint's scope manager (`sourceCode.getScope()`, `variable.references`) has no direct equivalent, but the common case — "every identifier that references this declared symbol" — is served by `ctx.Refs.References(sym)`, a lazily built per-file reference index keyed by binder symbols (`decl.Symbol()`). The reverse direction — "what symbol does this identifier resolve to" — is `ctx.Refs.Resolve(node)` (same-file only, never touches the checker) or `ctx.Refs.ResolveWithChecker(node)` (also resolves globals/ambient/cross-file symbols, at the cost of a checker round-trip per identifier the binder can't place). Use these instead of walking the AST and calling `ctx.TypeChecker.GetSymbolAtLocation` per identifier, which is a known performance killer — and never hand-roll a "try ctx.Refs, fall back to the checker" wrapper, since `ResolveWithChecker` already is one. See [AST_PATTERNS.md — Resolving Identifiers and Collecting References](./AST_PATTERNS.md#resolving-identifiers-and-collecting-references-ctxrefs) for semantics and the nil guard.
...
- `IsShadowed`, `FindEnclosingScope`, `CollectBindingNames` — scope / binding queries. For "all references to this declared symbol" use `ctx.Refs.References(decl.Symbol())`, and for "what does this identifier resolve to" use `ctx.Refs.Resolve(node)` / `ctx.Refs.ResolveWithChecker(node)` for symbols possibly declared outside this file — never a hand-rolled AST walk with `GetSymbolAtLocation` per identifier (see [AST_PATTERNS.md — Resolving Identifiers and Collecting References](AST_PATTERNS.md#resolving-identifiers-and-collecting-references-ctxrefs))
