...
// behind golang.org/x/text's caser. Node 26 carries ICU 78, which is
// Unicode 17.0. Across the editions in between, ten characters that had no
// case mapping at all were given one, two bicameral scripts arrived whole, and
// the two properties that decide where a Greek capital sigma lowercases to a
// final sigma — Cased and Case_Ignorable — took on a hundred and ninety more
// characters and lost two. Without the data here, two characters JavaScript
// compares equal under a regexp's `iu` flags compare unequal, a string comes
// back from a case mapping JavaScript would have changed, and a final sigma
// lands on the wrong one of ς and σ.
//
...
// a table generator.
//
// TODO(go1.27): delete this package and drop its imports — the four lookups in
// ecmascript's isCased, isCaseIgnorable, toLower and unicode17Uppercase, and the
// three in ecmascript/regexp's Canonicalize, simpleFold and buildCaseTables.
// golang.org/x/text v0.40.0 already ships Unicode 17.0.0 tables behind a
// `//go:build go1.27` constraint, so the standard library is expected to reach
// 17.0.0 in that release. TestDeltaStillNeeded fails once the toolchain gets
// there, so this does not rely on anyone remembering.
package unicode17
