...
func (staticEvaluator *StaticStringEvaluator) evalBuiltinStaticCall(node *ast.Node) staticEvalResult {
	...
	switch method {
	case "toUpperCase":
		// x/text's root-locale uppercase conversion follows ECMAScript's Unicode
		// Default Case Conversion for String.prototype.toUpperCase.
		return staticEvalResult{value: cases.Upper(language.Und).String(text), ok: true}
	case "toLowerCase":
		return staticEvalResult{value: cases.Lower(language.Und).String(text), ok: true}
	case "slice":
...
func toUint32(number float64) uint32 {
	...
	return uint32(math.Mod(math.Trunc(number), 1<<32))
}
