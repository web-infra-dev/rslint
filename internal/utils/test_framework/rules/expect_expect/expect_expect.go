...
type Config struct {
	...
	// ClassifyTest reports whether node is a test registration and whether it is
	// a `.todo` form. Called for every CallExpression.
	ClassifyTest func(node *ast.Node, ctx rule.RuleContext) TestClassification
	// ResolveNamedCallback resolves a by-reference callback for a test call, so
	// that an assertion inside the referenced function counts for the test.
	// Returning a zero NamedCallback means the call has no resolvable named
	// callback (e.g. it uses an inline function, which is handled by the
	// ancestor walk instead).
	ResolveNamedCallback func(callExpr *ast.CallExpression, ctx rule.RuleContext) NamedCallback
}
		...
		Run: func(ctx rule.RuleContext, options []any) rule.RuleListeners {
			...
			return rule.RuleListeners{
				ast.KindCallExpression: func(node *ast.Node) {
					...
					if config.ClassifyTest != nil {
						classification = config.ClassifyTest(node, ctx)
					}
					...
					if isTest || isExtraBlock {
						...
						if isTest && config.ResolveNamedCallback != nil {
							named := config.ResolveNamedCallback(callExpr, ctx)
							switch {
