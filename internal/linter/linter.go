...
// runLintRulesInFile is the shared traversal for retained and deferred sources.
// The calling worker owns the listener registry and reuses it only after reset.
func runLintRulesInFile(
	sourceProgram *program.Program,
	filePlan *lintFilePlan,
	rules []rule.ConfiguredRule,
	chk *checker.Checker,
	opts programRunOptions,
	consumer rule.DiagnosticConsumer,
	registeredListeners *listenerRegistry,
) {
	file := filePlan.file
	...
	queueFiles := func(chk *checker.Checker, tasks []lintFileTask) {
		wg.Queue(func() {
			...
			for _, task := range tasks {
				runLintRulesInFile(sourceProgram, task.plan, task.rules, chk, opts, consumer, &registeredListeners)
			}
		})
	}
...
// RunLinterContext executes a plan with caller-owned cancellation. Deferred
// workers stop admitting new files on cancellation and join before returning.
func RunLinterContext(ctx context.Context, opts RunLinterOptions) (*LintResult, error) {
	...
	if opts.LintPlan != nil {
		...
		deferred, err := runDeferredFiles(ctx, plan.deferredFiles, runOpts, consumer)
		if err != nil {
			return nil, err
		}
		mergeResult(deferred)
		hasSyntacticDiagnostics = hasSyntacticDiagnostics || deferred.hasSyntacticDiagnostics
	}
	...
	return &LintResult{
		...
		HasSyntacticDiagnostics: hasSyntacticDiagnostics,
	}, ctx.Err()
}
