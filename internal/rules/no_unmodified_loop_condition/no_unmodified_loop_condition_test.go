...
func TestNoUnmodifiedLoopConditionRule(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			// Group semantics: a < b is one group, if a is modified, b is OK too
			...
			// OR groups: a || b — if a is modified in one group, that group is OK
			{Code: `var a = 0, b = 0; while (a || b) { a++; b++; }`},
		},
		// Invalid cases
		[]rule_tester.InvalidTestCase{
			// While statement - simple identifier not modified
			{
				Code: `var foo = 0; while (foo) { } foo = 1;`,
			...
			// Both identifiers unmodified in group
			{
			...
			// OR groups: both groups unmodified
			{
				Code: `var a = 0, b = 0; while (a || b) { }`,
				Errors: []rule_tester.InvalidTestCaseError{
					{MessageId: "loopConditionNotModified", Line: 1, Column: 26},
