...
func TestNoUnreachableRule(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			// If/else where both branches terminate — subsequent code is unreachable
			// but this is tested in invalid cases
			// Try/catch where try terminates and no catch
			{Code: `function foo() { try { return; } finally { cleanup(); } }`},
		},
		// Invalid cases
		[]rule_tester.InvalidTestCase{
			...
			// Unreachable after if/else where both branches return
			{
				...
				Errors: []rule_tester.InvalidTestCaseError{
					{MessageId: "unreachableCode", Line: 1, Column: 58},
				},
			},
			// Unreachable after try/catch where both terminate
			{
				Code: "function foo() { try { return 1; } catch(e) { return 2; } bar(); }",
				Errors: []rule_tester.InvalidTestCaseError{
					{MessageId: "unreachableCode", Line: 1, Column: 59},
				},
			},
		},
	)
