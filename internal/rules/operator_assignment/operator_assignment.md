...
- In `"never"` mode, `<<=` is reported without a fix when the right side both needs parentheses and contains a `<` — for example `x <<= foo<T>`. TypeScript re-scans the `<` of `<<` as the start of a type argument list, so `x = x << (foo<T>)` would not parse.
