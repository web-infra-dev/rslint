...
func TestNoRestrictedGlobalsExtras(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			// ---- Branch lock-in: all case/default clauses of a switch share one
			// CaseBlock scope, so a `let` in a later clause shadows a reference to
			// the same name in an earlier clause ----
			{
				Code:    `switch (x) { case 0: foo; break; case 1: let foo; }`,
				Options: []interface{}{"foo"},
			},

			// ---- Dimension 4: JSX tag names — a lowercase/hyphenated tag name
			// (`<foo />`) names an intrinsic element, not a variable reference —
			// it must not be checked even when "foo" is restricted ----
			{
			...
			// ---- Branch lock-in: an empty-string entry in `globals` is not a
			// valid identifier and must be filtered out rather than restricting
			// the empty property name ----
			{
				Code:    `window[""];`,
		...
		[]rule_tester.InvalidTestCase{
			// ---- Optimization lock-in: SourceFile.HasIdentifier uses normalized
...
window.foo();`,
			...
			// ---- Branch lock-in: a default parameter value is evaluated in the
			// parameter environment, which is a *parent* of the function body's
			// variable environment — a `var` in the body does not shadow a
			// reference in a default value ----
			{
			...
			// ---- Dimension 4: JSX tag names — a capitalized tag name is a real
			// reference to a component variable and must still be checked ----
			{
				Code:    `const el = <Foo />;`,
				Options: []interface{}{"Foo"},
				Tsx:     true,
			...
			// ---- Dimension 4: access/key forms — BigInt computed keys beyond
			// int64 range must still normalize to their full decimal string, not
			// silently degrade to raw literal text ----
			{
