...
// Message builders
func buildThisBeforeSuper() rule.RuleMessage {
...
func buildSuperBeforeSuper() rule.RuleMessage {
	return rule.RuleMessage{
		Id:          "superBeforeSuper",
...
func checkStatements(stmts []*ast.Node, ctx *rule.RuleContext) superStatus {
	status := superNone
...
func checkExpressionForViolations(expr *ast.Node, status superStatus, ctx *rule.RuleContext) {
	...
	case ast.KindThisKeyword:
		ctx.ReportNode(expr, buildThisBeforeSuper())
		return
	...
	case ast.KindSuperKeyword:
		...
		ctx.ReportNode(expr, buildSuperBeforeSuper())
		return
...
func checkNodeForViolations(node *ast.Node, status superStatus, ctx *rule.RuleContext) {
	...
	switch node.Kind {
	case ast.KindThisKeyword:
		ctx.ReportNode(node, buildThisBeforeSuper())
		return
	...
	case ast.KindSuperKeyword:
		...
		ctx.ReportNode(node, buildSuperBeforeSuper())
		return
