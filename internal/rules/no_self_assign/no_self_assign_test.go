...
func TestNoSelfAssignRule(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			{Code: `({a} = {a: b})`},
			// Unary minus produces a different value
			{Code: `a = -a`},
			// Different object, same property name
			{Code: `a.b = c.b`},
			// Destructuring pattern vs plain identifier are structurally different
			{Code: `({a} = a)`},
			// Object destructuring with different property values
			{Code: `({a: b} = {a: c})`},
			// Spread into a new object creates a copy
			{Code: `a = {...a}`},
			// Properties after a spread on the right are not matched
			{Code: `({a, ...b} = {a, ...b})`},
			{Code: `({a, ...b} = {c, ...b})`},
		},
		[]rule_tester.InvalidTestCase{
			...
			// Object destructuring self-assignments
			{
				Code: `({a} = {a})`,
				Errors: []rule_tester.InvalidTestCaseError{
			...
			// Chained property access self-assignment
			{
				Code: `a.b.c = a.b.c`,
				Errors: []rule_tester.InvalidTestCaseError{
					{MessageId: "selfAssignment", Line: 1, Column: 9},
			...
			// Same numeric index self-assignment
			{
				Code: `a[0] = a[0]`,
				Errors: []rule_tester.InvalidTestCaseError{
					{MessageId: "selfAssignment", Line: 1, Column: 8},
			...
			// Object destructuring with renamed property self-assignment
			{
				Code: `({a: b} = {a: b})`,
				Errors: []rule_tester.InvalidTestCaseError{
					{MessageId: "selfAssignment", Line: 1, Column: 15},
