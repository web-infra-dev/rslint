	...
	Run: func(ctx rule.RuleContext, options any) rule.RuleListeners {
		...
		listeners[ast.KindBinaryExpression] = func(node *ast.Node) {
			...
			case ast.KindPlusEqualsToken, ast.KindMinusEqualsToken,
				...
				// Compound assignment operators (only when option is enabled)
				if opts.disallowArithmeticOperators {
					if hasOptionalChain(bin.Right) {
						ctx.ReportNode(node, unsafeArithmeticMsg)
					}
				}
			}
		...
		// Exception: spread in object expression is safe ({...obj?.foo} just spreads nothing)
		listeners[ast.KindSpreadElement] = func(node *ast.Node) {
			se := node.AsSpreadElement()
			if se == nil {
				return
			}
			// Skip spread in object literals — spreading undefined into {} is safe
		...
		classHandler := func(node *ast.Node) {
			...
			for _, clause := range heritageClauses.Nodes {
				...
				for _, typeNode := range hc.Types.Nodes {
					...
					if exprWithTypeArgs == nil {
						continue
					}
					reportIfOptionalChain(typeNode, exprWithTypeArgs.Expression, unsafeOptionalChainMsg)
				}
			}
		}
...
func hasOptionalChain(node *ast.Node) bool {
	...
	case ast.KindNonNullExpression:
		// NonNullExpression wraps an inner expression (e.g., `(obj?.foo)!`),
		// so we must recurse into it to find the optional chain.
		nne := node.AsNonNullExpression()
		if nne == nil {
			return false
		}
		return hasOptionalChain(nne.Expression)
	...
	case ast.KindAwaitExpression:
		...
		return hasOptionalChain(ae.Expression)
