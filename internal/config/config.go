...
func (config RslintConfig) GetConfigForFile(filePath string) *MergedConfig {
	merged := &MergedConfig{
...
func isFileMatched(filePath string, patterns []string) bool {
	cwd, err := os.Getwd()
	if err != nil {
		cwd = ""
	}
...
import { ts } from '@rslint/core/configs';
...
import { js } from '@rslint/core/configs';
...
// If tsconfig.json exists, generates rslint.config.ts; otherwise generates rslint.config.js.
func InitDefaultConfig(directory string) error {
	...
	if _, err := os.Stat(tsconfigPath); err == nil {
		...
		configPath := filepath.Join(directory, "rslint.config.js")
		if err := os.WriteFile(configPath, []byte(defaultJSConfig), 0644); err != nil {
			return fmt.Errorf("failed to create rslint.config.js: %w", err)
		}
		fmt.Println("Created rslint.config.js with JavaScript recommended config.")
	}
