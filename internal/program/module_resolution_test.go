...
func TestResolveNodeModuleGenerationAndOptions(t *testing.T) {
	...
	for _, test := range []struct {
		name    string
		options lintprogram.NodeResolutionOptions
		want    string
	}{
		...
		{"conditional", lintprogram.NodeResolutionOptions{}, "/node-runtime/node_modules/conditional/import.js"},
		{"conditional", lintprogram.NodeResolutionOptions{Conditions: []string{"types"}}, "/node-runtime/node_modules/conditional/index.d.ts"},
	} {
		if got := p.ResolveNodeModule(test.name, "/node-runtime/input.js", test.options); got != test.want {
			t.Errorf("ResolveNodeModule(%s, %#v) = %q, want %q", test.name, test.options, got, test.want)
		}
	}
	var group sync.WaitGroup
	for range 8 {
		group.Go(func() {
			if got := p.ResolveNodeModule("pkg", "/node-runtime/input.js", lintprogram.NodeResolutionOptions{}); got != "/node-runtime/node_modules/pkg/index.js" {
	...
	if got := p.ResolveNodeModule("new-package", "/node-runtime/input.js", lintprogram.NodeResolutionOptions{}); got != "" {
		t.Errorf("previous generation saw a new file: %q", got)
	}
	if got := create().ResolveNodeModule("new-package", "/node-runtime/input.js", lintprogram.NodeResolutionOptions{}); got != "/node-runtime/node_modules/new-package/index.js" {
...
func TestNearestCompilerOptionsGeneration(t *testing.T) {
	...
	p, _, _ := programForImport(t, files, "/nearest-config/input.ts")
	options := p.NearestCompilerOptions("/nearest-config/deep/input.ts")
	...
	if p.NearestCompilerOptions("/nearest-config/deep/input.ts") != options {
	...
	updated, _, _ := programForImport(t, files, "/nearest-config/input.ts")
	if got := updated.NearestCompilerOptions("/nearest-config/deep/input.ts"); got == nil || got.AllowImportingTsExtensions != core.TSFalse {
		t.Fatalf("config leaked between generations: %#v", got)
	}
	if (*lintprogram.Program)(nil).NearestCompilerOptions("/input.ts") != nil {
