...
type analyzer struct {
	...
	callbacks jestUtils.JestTestCallbacks
...
func buildFloatingPromiseMessage() rule.RuleMessage {
	return rule.RuleMessage{
		...
		Description: "This promise should either be returned or awaited to ensure the expects in its chain are called",
	}
}
...
var ValidExpectInPromiseRule = rule.Rule{
	...
	Run: func(ctx rule.RuleContext, _ []any) rule.RuleListeners {
		...
		for function := range callbacks.Functions {
			if callbacks.IgnoredDone[function] {
				continue
			}
...
func (a *analyzer) isAssertionCall(node *ast.Node) bool {
	parsed := a.callbacks.ParseFnCall(node)
...
func (a *analyzer) isAsyncExpectSink(callNode, identifier *ast.Node) bool {
	...
	parsed := a.callbacks.ParseFnCall(top)
	...
	head := leftMostCallExpression(top)
