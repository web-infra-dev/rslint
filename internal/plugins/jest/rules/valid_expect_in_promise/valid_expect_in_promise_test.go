	...
	rule_tester.RunRuleTester(
		...
		[]rule_tester.InvalidTestCase{
			...
			// path through the right-hand side evaluates to it.
			{
				...
				Code: `test("case", async () => { let pending = promise.then(value => expect(value).toBe(1)); pending = condition ? pending : other; await pending; });`,
				Errors: []rule_tester.InvalidTestCaseError{{
					MessageId: "expectInFloatingPromise",
				}},
			},
			{
				...
				Code: `test("case", async () => { let pending = promise.then(value => expect(value).toBe(1)); pending = other || pending; await pending; });`,
				Errors: []rule_tester.InvalidTestCaseError{{
					MessageId: "expectInFloatingPromise",
				}},
			},
			...
			},
			{
				...
				Code: `test("case", async () => { let pending = promise.then(value => expect(value).toBe(1)); pending = ready && pending; await pending; });`,
				Errors: []rule_tester.InvalidTestCaseError{{
					MessageId: "expectInFloatingPromise",
				}},
			},
