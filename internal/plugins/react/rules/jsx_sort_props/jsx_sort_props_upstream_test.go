...
func TestJsxSortPropsUpstream(t *testing.T) {
	rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.json", t, &JsxSortPropsRule, []rule_tester.ValidTestCase{
		...
		{Code: `<App key={4} />`, Tsx: true, Options: map[string]any{"reservedFirst": []any{}}, Errors: []rule_tester.InvalidTestCaseError{{MessageId: "listIsEmpty", Message: "A customized reserved first list must not be empty", Line: 1, Column: 6, EndLine: 1, EndColumn: 13}}},
		{Code: `<App key={5} />`, Tsx: true, Options: map[string]any{"reservedFirst": []any{"notReserved"}}, Errors: []rule_tester.InvalidTestCaseError{{MessageId: "noUnreservedProps", Message: "A customized reserved first list must only contain a subset of React reserved props. Remove: notReserved", Line: 1, Column: 6, EndLine: 1, EndColumn: 13}}},
