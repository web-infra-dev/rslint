...
React Server Actions must be async even when they do not await anything or
return a promise. This rule checks function declarations, function expressions,
...
Only the first statement is checked. A preceding statement, including another
directive such as `"use strict"`, prevents a match. Generators and template
literals are ignored. A file-level `"use server"` directive does not cause this
rule to check all exported functions.

The rule offers an editor suggestion to insert `async`; it does not apply an
automatic fix.
...
This rule follows the upstream commit linked below; it is not included in upstream
release `v7.37.5`.
