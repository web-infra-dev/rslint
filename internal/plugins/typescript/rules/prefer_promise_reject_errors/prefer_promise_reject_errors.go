	...
	Run: func(ctx rule.RuleContext, options []any) rule.RuleListeners {
		...
		checkRejectCall := func(callExpression *ast.CallExpression) {
			if len(callExpression.Arguments.Nodes) != 0 {
				...
				if utils.IsErrorLike(ctx.TypeScriptProgram(), ctx.TypeChecker, t) || utils.IsReadonlyErrorLike(ctx.TypeScriptProgram(), ctx.TypeChecker, t) {
					return
				}
		...
		typeAtLocationIsLikePromise := func(node *ast.Node) bool {
			...
			return (utils.IsPromiseConstructorLike(ctx.TypeScriptProgram(), ctx.TypeChecker, t) || utils.IsPromiseLike(ctx.TypeScriptProgram(), ctx.TypeChecker, t))
		}
		...
		return rule.RuleListeners{
			ast.KindCallExpression: func(node *ast.Node) {
				...
				if ast.IsAccessExpression(callee) {
					...
					if !utils.IsPromiseConstructorLike(ctx.TypeScriptProgram(), ctx.TypeChecker, ctx.TypeChecker.GetTypeAtLocation(parentNode.Expression())) {
						return
					}
