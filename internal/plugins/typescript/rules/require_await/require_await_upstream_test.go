// TestRequireAwaitUpstream migrates the typescript-eslint v8.68.0 require-await suite and its ESLint base-rule cases.
// rslint-specific lock-in cases live in require_await_extras_test.go.
...
func requireAwaitLeadingSpaces(value string) int {
...
func alignRequireAwaitSuggestionIndent(cases []rule_tester.InvalidTestCase) []rule_tester.InvalidTestCase {
	for caseIndex := range cases {
		codeIndent := requireAwaitLeadingSpaces(cases[caseIndex].Code)
		for errorIndex := range cases[caseIndex].Errors {
			for suggestionIndex := range cases[caseIndex].Errors[errorIndex].Suggestions {
				suggestion := &cases[caseIndex].Errors[errorIndex].Suggestions[suggestionIndex]
				difference := codeIndent - requireAwaitLeadingSpaces(suggestion.Output)
				if difference <= 0 || !strings.HasPrefix(suggestion.Output, "\n") {
					continue
				}
				lines := strings.Split(suggestion.Output, "\n")
				prefix := strings.Repeat(" ", difference)
				for lineIndex := 1; lineIndex < len(lines); lineIndex++ {
					lines[lineIndex] = prefix + lines[lineIndex]
				}
				suggestion.Output = strings.Join(lines, "\n")
			}
		}
	}
	return cases
...
async function* foo(): Promise<string> {
  return new Promise(res => res(` + "`" + `hello` + "`" + `));
    ...
    `},
	}, alignRequireAwaitSuggestionIndent([]rule_tester.InvalidTestCase{
		{
			Code: `
async function numberOne(): Promise<number> {
  return 1;
...
// TestRequireAwaitUpstreamEslintBase migrates the applicable ESLint v10.9.1 base-rule cases.
func TestRequireAwaitUpstreamEslintBase(t *testing.T) {
	rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.json", t, &RequireAwaitRule, []rule_tester.ValidTestCase{
		{Code: `
async function foo() {
  await doSomething();
