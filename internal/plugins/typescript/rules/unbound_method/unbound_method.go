...
var UnboundMethodRule = rule.CreateRule(rule.Rule{
	...
	Run: func(ctx rule.RuleContext, options []any) rule.RuleListeners {
		...
		isNativelyBound := func(object *ast.Node, property *ast.Node) bool {
			...
			return utils.IsBuiltinSymbolLike(ctx.TypeScriptProgram(), ctx.TypeChecker, ctx.TypeChecker.GetTypeAtLocation(object), supportedGlobalTypes...) && utils.IsAnyBuiltinSymbolLike(ctx.TypeScriptProgram(), ctx.TypeChecker, ctx.TypeChecker.GetTypeAtLocation(property))
		}
