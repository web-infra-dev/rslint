...
func TestFilenameCase(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			// for every case style. The Go-side rule tester carries only serialized
			// strings, so we migrate those variants and skip live JS RegExp values.
			{Code: `// undef-ignore`, Options: withIgnore(caseOpt("kebabCase"), `FOOBAR\.js`)},
			{Code: `// undef-ignore`, Options: withIgnore(caseOpt("kebabCase"), `FOOBAR\.js`),
				Skip: true /* SKIP: ignore live JS RegExp variant */},
