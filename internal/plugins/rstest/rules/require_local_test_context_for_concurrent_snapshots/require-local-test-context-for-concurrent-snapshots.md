...
An explicit inner `sequential` or `concurrent` registration overrides the mode inherited from an enclosing describe block. The rule recognizes Rstest aliases, named callbacks, `.each`, `.for`, `import.meta.rstest`, and `@rstest/playwright` registrations.
...
A helper declared outside every registration callback is not checked, since the rule cannot tell which tests call it.
