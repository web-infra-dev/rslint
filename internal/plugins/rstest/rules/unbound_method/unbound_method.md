...
If the file replaces a built-in matcher with `expect.extend()`, that matcher is not exempt. Equality-based matchers are likewise not exempt after the file calls `expect.addEqualityTesters()`, or when a value compared directly with the received method is a custom asymmetric matcher with a callable `asymmetricMatch` member, because that user code can execute the received function. The invoking built-in factories `expect.toSatisfy()` and `expect.schemaMatching()` remain diagnostic, and `toBeOneOf()` also remains diagnostic when a direct array or `Set` sample may invoke the received method. Built-in asymmetric matchers with known non-invoking behavior remain exempt. These checks also follow stable local aliases of matcher values and collections. Extensions and equality testers loaded only through other files are not detected.
