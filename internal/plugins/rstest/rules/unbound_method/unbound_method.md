...
If the file replaces a built-in matcher with `expect.extend()`, that matcher is not exempt. Equality-based matchers are likewise not exempt after the file calls `expect.addEqualityTesters()`, because the custom matcher or equality tester can execute the received function. These checks also follow local aliases of `expect` and its configuration methods when the aliases are not reassigned; matcher extensions and equality testers loaded only through other files are not detected.
