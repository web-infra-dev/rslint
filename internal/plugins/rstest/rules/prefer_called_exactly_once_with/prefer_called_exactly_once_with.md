...
The last: an exempt default-library call may still run the iterator protocol on what it is handed. `Array.from(gen)` and `Promise.all(gen)` iterate a generator the way `[...gen]` does, but a generator object carries no call signature, so the test for a callable argument reports that nothing was handed over. Testing instead whether an argument can be iterated would take arrays and strings with it, and with those `console.log(items)` — the shape the exemption exists for — so this one is left open.
