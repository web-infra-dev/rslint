...
- a mock reset sits between them, at any nesting depth, because each assertion then describes a different call history. This covers `mockClear`, `mockReset`, and `mockRestore` called on the target — the receiver is matched against the target structurally, so parentheses, type assertions, and comments in either spelling do not hide it, and a receiver wrapped in `mocked(...)` is unwrapped first, since `mocked` is identity at runtime. It also covers `clearAllMocks`, `resetAllMocks`, and `restoreAllMocks`, which reset every mock and so need no receiver match. When the target is not stable enough to compare — `expect(getMock())` — any reset between the two assertions blocks the merge, whatever it is called on;
