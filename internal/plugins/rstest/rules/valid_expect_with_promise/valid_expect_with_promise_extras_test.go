...
func TestValidExpectWithPromiseExtras(t *testing.T) {
	...
	valid := []rule_tester.ValidTestCase{
		...
		{Code: `(expect(1) as any).toBe(1).result.resolves.toBe(1);`},
	...
	for _, code := range []string{
		...
		`class PromiseWithValue extends Promise<number> { value = 1 } declare const subject: PromiseWithValue; expect(subject).to.have.property('value').and.resolves.toBe(1);`,
		`class PromiseWithValue extends Promise<number> { value = 1 } declare const subject: PromiseWithValue; (expect(subject) as any).to.have.property('value').and.resolves.toBe(1);`,
		`class PromiseWithValue extends Promise<number> { value = 1 } async function check(subject: PromiseWithValue) { await (expect(subject) as any).to.have.property('value').and.resolves.toBe(1); }`,
		`function required(value: string): Promise<never> { return Promise.reject(new Error(value)); } expect(required).rejects.toThrow();`,
		`declare function required(...args: [string]): Promise<never>; expect(required).rejects.toThrow();`,
