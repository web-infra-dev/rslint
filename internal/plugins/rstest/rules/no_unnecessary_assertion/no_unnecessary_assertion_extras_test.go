...
func TestNoUnnecessaryAssertionRstestMatchers(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			{Code: "declare function read(value?: number): null; expect.poll(read).toBeNull();"},
			{Code: "declare function read(...values: number[]): null; expect.poll(read).toBeNull();"},
			{Code: "declare function read(value: void): null; expect.poll(read).toBeNull();"},
			{Code: "declare function read(...values: [number?]): null; expect.poll(read).toBeNull();"},
			{Code: "declare function read(): string; declare function read(value?: number): null; expect.poll(read).toBeNull();"},
			{Code: "declare function read(value: number): string; expect.poll(read).toBeNull();"},
		...
		[]rule_tester.InvalidTestCase{
			...
			{Code: "declare function read(): string; declare function read(value: number): null; expect.poll(read).toBeNull();", Errors: []rule_tester.InvalidTestCaseError{diagnostic("null", 1)}},
			{Code: "declare function read(): string; declare function read(...values: [number]): null; expect.poll(read).toBeNull();", Errors: []rule_tester.InvalidTestCaseError{diagnostic("null", 1)}},
