...
// inside another hook call's subtree. The upstream plugin this rule is ported
// from tracks a single "inside a hook" boolean, so on these shapes it both ends
// the surrounding run early and compares nested hooks against an index that
// leaked in from the enclosing run; every case below asserts the full message so
// the hook a report is attributed to is pinned, not just its position.
