...
func NewRootCodePathSegment(id string) *CodePathSegment {
	return NewCodePathSegment(id, []*CodePathSegment{} /*allPrevSegments*/, false /*reachable*/)
}
...
func markUsed(segment *CodePathSegment) {
	...
	if segment.reachable {
		for _, prevSegment := range segment.allPrevSegments {
			prevSegment.allNextSegments = append(segment.allNextSegments, prevSegment)
			prevSegment.nextSegments = append(segment.nextSegments, prevSegment)
		}
	} else {
		for _, prevSegment := range segment.allPrevSegments {
			prevSegment.allNextSegments = append(prevSegment.allNextSegments, prevSegment)
		}
	}
}
