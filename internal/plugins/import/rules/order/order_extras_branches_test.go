...
func TestOrderBranchCoverage(t *testing.T) {
	rule_tester.RunRuleTester(
		...
		[]rule_tester.ValidTestCase{
			...
			// Locks in upstream create().TSImportEqualsDeclaration arm 1: exported import-equals is ignored.
			{Code: "export import sibling = require('./z');\nimport fs = require('fs');"},
			// TSESTree wraps an exported variable declaration, so its require call
			// never reaches getRequireBlock. tsgo stores export as a modifier.
			{
				Code:    "export const sibling = require('./z');\nconst fs = require('fs');",
				Options: map[string]any{"alphabetize": map[string]any{"order": "asc"}},
			},
			// Empty named bindings produce zero ESTree specifiers and are ignored
			// unless warnOnUnassignedImports is enabled.
			{
				Code:    "import {} from './z';\nimport a from './a';",
				Options: map[string]any{"alphabetize": map[string]any{"order": "asc"}},
			},
			...
			// Locks in upstream create().AssignmentExpression arm 1: exportMap is not sorted when alphabetize is ignore.
			{
				Code:    "exports.Z = 1;\nmodule.exports.A = 1;",
				Options: map[string]any{"named": map[string]any{"cjsExports": true}},
			},
			// Locks in upstream isCJSExports() arm 1: local module/exports bindings suppress CJS handling.
			{
		...
		[]rule_tester.InvalidTestCase{
			// fixOutOfOrder mutates duplicate display names while disambiguating
			// aliases. Later diagnostics sharing the first entry observe that state.
			{
			...
			// a scalar and again in a nested group, upstream's later rank wins.
			{
			...
			// fails. importType performs that check before its relative-path arms.
			{
				Code: "import external from 'package-name';\n" +
			...
			// Upstream's local isRequireExpression accepts every ESTree Literal,
			// not just module-name strings, for named destructuring.
			{
			...
			// Optional chains are wrapped in ESTree ChainExpression. Although tsgo
			// stores the outer call directly, it must remain an autofix barrier.
			{
			...
			// Locks in upstream makeOutOfOrderReport() arm 1: reverse scan, including its no-final-newline replacement.
			{
			...
			// Locks in upstream makeOutOfOrderReport() arm 2: named reverse scan and the `after` fixer.
			{
			...
			// Locks in upstream fixOutOfOrder() arm 1: aliases disambiguate equal display names.
			{
			...
			// Locks in upstream getSorter() arm 1: descending import-kind secondary comparison.
			{
			...
			// Locks in upstream computePathRank() arm 1: a pathGroup without an explicit position.
			{
			...
			// Locks in upstream convertPathGroupsForRanks() arm 1: before-group count rounds to a power of ten.
			{
			...
			// Locks in upstream computePathRank() arm 2: patternOptions combine matchBase, nocase, and extglob.
			{
			...
			// Locks in upstream canReorderItems() arm 1: a multi-declarator require reports without a fix.
			{
			...
			// The same ESTree Literal predicate makes a non-string require a safe
			// fixer barrier even though it is not itself registered as a module.
			{
...
// TestOrderCommonJSControlFlowFixRoots locks in upstream findRootNode's ESTree
// `body` walk. An unbraced if/else pair and assignments in one switch clause
// share an export sequence, but both entries resolve to the same movable root;
// the first fix therefore only normalizes the trailing newline. A braced block
// keeps each ExpressionStatement as its own movable root and can be sorted.
func TestOrderCommonJSControlFlowFixRoots(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !fixed {
				t.Fatal("expected an upstream-compatible fix")
			}
			if output != test.want {
