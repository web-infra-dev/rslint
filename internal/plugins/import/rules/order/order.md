...
`patternOptions` accepts the matching controls supported by minimatch 3, such
as `dot`, `matchBase`, `nobrace`, `nocase`, `nocomment`, `noext`,
`noglobstar`, `nonegate`, `partial`, and `flipNegate`.
...
| `import/external-module-folders` | Resolved paths under any of these folders classify as `external` (default `["node_modules"]`). An explicit `[]` disables that default; `""` denotes the package root. |
