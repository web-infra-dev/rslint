...
func TestIsExternalModulePathFromPackage(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "outside package is external", packagePath: "/repo/packages/app", resolvedPath: "/repo/shared/pkg/index.js", caseSensitive: true, want: true},
		...
		{name: "empty folder means package root", settings: map[string]interface{}{"import/external-module-folders": []string{""}}, packagePath: "/repo", resolvedPath: "/repo/src/local.js", caseSensitive: true, want: true},
		{name: "absolute folder prefix is not enough", settings: map[string]interface{}{"import/external-module-folders": []string{"/dependencies"}}, packagePath: "/", resolvedPath: "/dependencies-extra/pkg/index.js", caseSensitive: true, want: false},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got := compiled.IsExternalPathFromPackage(test.packagePath, test.resolvedPath, test.caseSensitive); got != test.want {
				t.Fatalf("ModuleSettings.IsExternalPathFromPackage() = %v, want %v", got, test.want)
			}
		})
	}
}
