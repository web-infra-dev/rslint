// Command gen-rule-types dumps every registered native rule's name and
// options JSON Schema as JSON on stdout, for
// packages/rslint/scripts/generate-rule-option-types.mjs to compile into
// TypeScript types via json-schema-to-typescript. It is a standalone dev
// tool (run via `go run ./cmd/gen-rule-types`), not part of the shipped
// cmd/rslint binary.
package main
