...
With `fix: true`, rslint runs up to ten lint-fix passes. Result messages and counts describe the final fixed source; findings removed by applied fixes do not remain in `messages`.
