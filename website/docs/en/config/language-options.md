...
Each explicit project must support the lint target's filename extension before it can supply types. JavaScript requires an effective `allowJs` value of `true`, including when enabled by `checkJs`. A project that does not support the target's extension is skipped for that target, even if another target or `--type-check` causes the project to be loaded. Physical aliases do not bypass this check. Supported files brought in through imports can still receive types. The separate `projectService` behavior for explicitly listed JavaScript roots described above is unchanged.

No-argument, directory, explicit-file, API and editor lint requests use the same project selection and fallback rules. Project path resolution still checks that declared paths exist and glob patterns match. Config metadata may be read concurrently ahead of selection, but results are considered in project-list order. An unreadable config reports an error only if selection reaches it; an unused prefetched config does not. Prefetching does not construct a project's dependency graph or change which project supplies a target. After selecting a direct root and finding its source, later candidates are not considered for that target. Import lookup stops once all selected targets have an owner. Projects may contain files outside the lint range without adding those files to lint execution.
