...
No-argument, directory, explicit-file, API and editor lint requests use the same project selection and fallback rules. Project path resolution still checks that declared paths exist and glob patterns match. Config contents and dependency graphs are loaded only as needed: after selecting a direct root and finding its source, Rslint does not read later configs for that target. An unreadable config reports an error if selection reaches it; an unused later config does not. Import lookup stops once all selected targets have an owner. Projects may contain files outside the lint range without adding those files to lint execution.
