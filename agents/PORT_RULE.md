...
1.  **Checkout Main**: Ensure your workspace is on the latest `main` branch code before creating a new branch.
...
- **Content Requirement**:
  - **Rule Details**: A clear explanation of what the rule does (copy from ESLint "Rule Details" section).
  - **Incorrect Examples**: Code snippets that trigger errors, preceded by "Examples of **incorrect** code for this rule:".
  - **Correct Examples**: Code snippets that are valid, preceded by "Examples of **correct** code for this rule:".
  - **Original Link**: The URL to the official ESLint rule documentation.
...
2.  **Plugin Rules**:
    ...
    - **Distinction of References**:
      - **Directory & Config**: Refer to `packages/rslint-test-tools/tests/eslint-plugin-import` (as per Step 5). You **MUST** ensure `rslint.json` and `tsconfig` files are present.
...
3.  Paste the **exact same** test cases from Phase 1 (JS syntax).
    ...
    - **Type Matching**: You **MUST** ensure that `valid` and `invalid` test cases match the `ValidTestCase` and `InvalidTestCase` types defined in `rule-tester.ts`. Mismatched types (e.g., missing required fields) might be ignored by the test runner, leading to false positives.
...
### Step 9: Configure Test Environment (Conditional)
...
- **If this file is NOT used**: The rule registration in Step 8 (`internal/config/config.go`) is sufficient; ensure the test file is included in `rstest.config.mts`.
...
4.  **Project-wide Checks (Mandatory)**:
    - Ensure the project's type checking and linting pass.
    - **Command**: Run from the project root:
      ```bash
      pnpm typecheck && pnpm lint
      ```
    - **Action**: Fix any TypeScript or linting errors before submitting.
...
1.  **Configure Project Settings (Conditional)**:
    - **Check `rslint.json`**: Read `rslint.json` in the project root.
    - **Action**:
      - **If** the rule belongs to a plugin ALREADY configured in `plugins` (or is an ESLint Core rule, which is enabled by default):
        - Add the new rule to the `rules` object in `rslint.json` with severity `"warn"`.
        - Example: `"rules": { "new-rule-name": "warn", ... }`
      - **If** the plugin is NOT configured in `rslint.json`:
        - Do NOT modify `rslint.json`.

2.  **Commit Changes**:
    - **Constraint**: Commit **ONLY** changes related to the porting of this rule.
      - **Selectivity**: You **MUST** use specific file paths with `git add`. Do **NOT** use `git add .`. Only add files related to this port (implementation, tests, config, and `rslint.json` if modified).
