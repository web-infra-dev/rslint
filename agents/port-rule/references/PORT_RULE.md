...
New rules declare options as a **schema**, not as hand-rolled JSON parsing. Define `Schema0` (and `Schema1` for a second positional option, e.g. `eqeqeq`'s `["always", { "null": ... }]`) on the `rule.Rule` literal using the combinators in [internal/rule/schema.go](../../../internal/rule/schema.go):
