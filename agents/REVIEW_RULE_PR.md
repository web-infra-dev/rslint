...
1.  **Draft Findings**:
    - Compile all identified issues and potential fixes.
    - Present them to the user.

2.  **User Decision (MANDATORY)**:
    - **Ask**: "Should I **submit comments** to the PR, or **checkout and fix** the branch directly?"
    - **Wait** for the user's explicit instruction.

3.  **Execute Decision**:

    **Path A: Submit Comments**
    - **Action**: Post the findings as a comment on the PR.
      ```bash
      gh pr comment <PR_URL> --body "## Review Feedback\n\nFound a potential issue:\n- **Location**: [File/Line]\n- **Issue**: [Description]\n- **Suggestion**: [Fix]"
      ```

    **Path B: Checkout & Fix**
    - **Step 1: Checkout**: Switch to the PR branch and pull the latest changes.
      ```bash
      gh pr checkout <PR_URL>
      git pull
      ```
    - **Step 2: Apply Fixes**: Implement the code changes identified during analysis.
    - **Step 3: Submit**: Commit and push the changes directly.
      ```bash
      git add .
